...
@section('title', 'News & Announcements - SkillUp')
        ...
        <div class="max-w-4xl mx-auto px-4">
            ...
            <div class="space-y-6" data-feed>
                @forelse ($news ?? [] as $item)
                    ...
                    <div class="fade-item reveal" style="--i: {{ $loop->index }};" data-category="{{ $filterKey }}">
                        <div class="relative overflow-hidden rounded-3xl {{ $isUrgent ? 'urgent-pulse border-2 border-rose-200' : 'border border-[var(--paper-line)]' }} bg-white p-6 sm:p-8 shadow-sm signal-card group">
                            <div class="accent-bar absolute top-0 left-0 w-1.5 h-full bg-{{ $colorName }}-500"></div>
                            <div class="flex flex-col sm:flex-row gap-6">
                                <div class="flex-shrink-0 flex items-center justify-center w-12 h-12 rounded-full bg-{{ $colorName }}-50 text-{{ $colorName }}-{{ $isUrgent ? '500' : '600' }}">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex flex-wrap items-center justify-between gap-2 mb-2">
                                        <span class="inline-flex items-center gap-1.5 rounded-full bg-{{ $colorName }}-100 px-2.5 py-0.5 text-xs font-bold text-{{ $colorName }}-700 font-mono tracking-wide uppercase">
                                            ...
                                            @endif
                                            {{ ucfirst($item->category ?? 'Update') }}
                                        </span>
                                        <span class="text-xs font-mono text-slate-400">{{ optional($item->published_at ?? $item->created_at)->diffForHumans() }}</span>
                                    </div>
                                    <h3 class="font-display text-xl font-semibold text-slate-900 mb-3 group-hover:text-{{ $colorName }}-600 transition-colors duration-300">{{ $item->title }}</h3>
                                    <p class="font-body text-slate-600 leading-relaxed mb-4">
                                        {{ strip_tags($item->content) }}
                                    </p>
                                    @if(isset($item->video_file) || isset($item->featured_image))
                                        <a href="{{ route('news.show', $item->id) }}" class="arrow-link inline-flex items-center font-body font-medium text-{{ $colorName }}-600 hover:text-{{ $colorName }}-700 mb-4">
                                    ...
                                    <div class="flex items-center gap-1 mt-4 pt-4 border-t border-[var(--paper-line)]">
                                        ...
                                        <button type="button" onclick="toggleReaction(this, {{ $item->id }}, 'like')" class="reaction-btn p-1.5 px-2 rounded-full flex items-center gap-1.5 text-xs font-mono transition-colors {{ $userReaction === 'like' ? 'text-blue-600 bg-blue-50 ring-1 ring-blue-200' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800' }}">
                                            <span>👍</span>
                                            <span class="count">{{ $reactionCounts['like'] ?? 0 }}</span>
                                        </button>
                                        <button type="button" onclick="toggleReaction(this, {{ $item->id }}, 'heart')" class="reaction-btn p-1.5 px-2 rounded-full flex items-center gap-1.5 text-xs font-mono transition-colors {{ $userReaction === 'heart' ? 'text-rose-600 bg-rose-50 ring-1 ring-rose-200' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800' }}">
                                            <span>❤️</span>
                                            <span class="count">{{ $reactionCounts['heart'] ?? 0 }}</span>
                                        </button>
                                        <button type="button" onclick="toggleReaction(this, {{ $item->id }}, 'care')" class="reaction-btn p-1.5 px-2 rounded-full flex items-center gap-1.5 text-xs font-mono transition-colors {{ $userReaction === 'care' ? 'text-amber-600 bg-amber-50 ring-1 ring-amber-200' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800' }}">
                                            <span>🥰</span>
                                            <span class="count">{{ $reactionCounts['care'] ?? 0 }}</span>
                                        </button>
                                        <button type="button" onclick="toggleReaction(this, {{ $item->id }}, 'sad')" class="reaction-btn p-1.5 px-2 rounded-full flex items-center gap-1.5 text-xs font-mono transition-colors {{ $userReaction === 'sad' ? 'text-indigo-600 bg-indigo-50 ring-1 ring-indigo-200' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800' }}">
                                            <span>😢</span>
