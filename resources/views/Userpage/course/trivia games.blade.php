...
@section('title', 'Trivia Games - SkillUp')
@section('content')

{{--
    Expected (optional) variables from the controller — the page works with sane
    fallbacks if these aren't passed, but wire them up for real numbers:
      $leaderboard -> collection/array of { rank, name, score, games } sorted by score desc.
                      If omitted or empty, an honest "no scores yet" state is shown
                      instead of fake names.
      $gameStats   -> optional assoc array keyed by game slug, e.g.
                      ['multiple-choice' => ['players' => 132, 'rating' => 4.8], ...]
                      If a game's entry is missing, a "New" badge is shown instead
                      of a random invented number.
--}}
@php
...
<div class="trivia-page">
    <section class="trivia-hero">
        ...
        <div class="hero-stat">
            <strong>{{ $playableGames->count() }}</strong>
    ...
    <div class="games-grid" id="games-grid">
        @forelse ($playableGames as $game)
            @php $gameCourse = $game->module->course; $gameModule = $game->module; @endphp
            <div class="game-card" data-title="{{ $game->title }}" data-slug="{{ $game->slug }}">
                <div class="game-icon">🎯</div>
                <h3 class="game-title">{{ $game->title }}</h3>
                <p class="game-description">{{ $game->description ?: 'Test your knowledge and climb the leaderboard.' }}</p>
                <div class="game-stats">
                    <div class="game-stat">
                        <span class="game-stat-value">{{ $game->questions_count }}</span><span>Questions</span>
                    </div>
                    <div class="game-stat">
                        <span class="game-stat-value">{{ ucfirst($game->difficulty ?: 'Mixed') }}</span><span>Difficulty</span>
                    </div>
                </div>
                <a class="play-button" href="{{ route('quizzes.start', [$gameCourse->slug, $gameModule->slug, $game->slug]) }}"><i class="fa-solid fa-play"></i> Start Game</a>
            </div>
        @empty
    ...
    <div class="leaderboard">
        ...
        @if ($leaderboardRows->isNotEmpty())
            <table class="leaderboard-table">
                ...
                <tbody>
                    @foreach ($leaderboardRows as $entry)
                        @php $rank = $entry['rank'] ?? $entry->rank; @endphp
                        <tr>
                            <td>
                                <span class="rank {{ $rank === 1 ? 'first' : ($rank === 2 ? 'second' : ($rank === 3 ? 'third' : '')) }}">
                                    {{ $rank === 1 ? '🥇' : ($rank === 2 ? '🥈' : ($rank === 3 ? '🥉' : '#' . $rank)) }}
                                </span>
                            </td>
                            <td><span class="player-name">{{ $entry['name'] ?? $entry->name }}</span></td>
                            <td><span class="score">{{ number_format($entry['score'] ?? $entry->score) }}</span></td>
                            <td>{{ $entry['games'] ?? $entry->games }} games</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
...
<script>
(function () {
    var quickPlayScoreUrl = @json(route('trivia.quick-play.scores'));
    var canSubmitQuickPlayScore = @json(auth()->check());
    ...
    // `window.TRIVIA_BANK = @json($triviaBank ?? null)` line if you have real question sets.
    var MC_BANK = [
