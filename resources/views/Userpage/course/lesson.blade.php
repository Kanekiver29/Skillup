...
<div class="manual-stage">
    <div class="manual-cover">
        <div class="manual-spread">
            ...
            {{-- Rendered twice — recto and verso of the manual's title spread — so both pages stay in sync. --}}
            @foreach (['left', 'right'] as $side)
                <div class="manual-page {{ $side }}">
                    <span class="manual-eyebrow">Course manual</span>
                    <h1 class="manual-title">{{ $title }}</h1>
                    @if ($tagline)
                        <p class="manual-tagline">{{ $tagline }}</p>
                    @endif

                    <div class="manual-stats">
                        <div>
                            <div class="manual-stat-label">Modules</div>
                            <div class="manual-stat-value">{{ $modulesCount }}</div>
                        </div>
                        <div>
                            <div class="manual-stat-label">Lessons</div>
                            <div class="manual-stat-value">{{ $lessonsCount }}</div>
                        </div>
                        <div>
                            <div class="manual-stat-label">Progress</div>
                            <div class="manual-stat-value">{{ $progress }}%</div>
                        </div>
                    </div>

                    <div class="manual-actions">
                        <a href="{{ $tocUrl }}" class="manual-btn {{ $hasTocRoute ? '' : 'disabled' }}">Table of contents</a>
                        <a href="{{ $openUrl }}" class="manual-btn primary {{ $hasOpenRoute ? '' : 'disabled' }}">Open course</a>
                    </div>
                </div>
            @endforeach
