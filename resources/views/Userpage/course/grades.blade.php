...
<div class="container">
    ...
    <div class="grade-summary">
        <div class="summary-card">
            ...
            <div class="grade-value">{{ number_format($average ?? 0, 1) }}%</div>
    ...
    <div class="grades-table-wrapper">
        ...
        <table class="grades-table">
            ...
            <tbody>
                @forelse(($grades ?? collect()) as $attempt)
                    @php
                        $quiz = $attempt->quiz;
                    ...
                    <tr>
                        <td>{{ $title }}</td>
                        <td>Quiz{{ $quiz?->is_trivia ? ' / Trivia' : '' }}</td>
                        <td>{{ optional($attempt->completed_at)->format('M d, Y') }}</td>
                        <td>{{ number_format($score, 1) }}%</td>
                        <td><span class="grade-badge"><i class="fa-solid fa-star"></i> {{ $passed ? 'Passed' : 'Needs Work' }}</span></td>
                        <td><a class="status-badge {{ $passed ? 'status-passed' : 'status-failed' }}" href="{{ route('quizzes.results', [$quiz->module->course->slug, $quiz->module->slug, $quiz->slug, $attempt->id]) }}">{{ $passed ? '✓ Passed' : '✕ Needs Work' }} · Review</a></td>
                    </tr>
                @empty
    ...
    <div class="chart-wrapper">
        ...
        <a href="{{ route('courses.index') }}" class="action-button">
