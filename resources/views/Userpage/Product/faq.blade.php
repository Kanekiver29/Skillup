...
<div class="min-h-screen bg-slate-50 px-4 pb-20 pt-32 text-slate-900">
    <div class="mx-auto max-w-4xl">
        ...
        <div class="space-y-4">
            @foreach([
                ...
                <details class="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                    <summary class="flex cursor-pointer list-none items-center justify-between gap-4 font-display text-lg font-bold">{{ $question }} <i class="fas fa-plus text-sm text-blue-600 transition group-open:rotate-45"></i></summary>
                    <p class="mt-4 max-w-3xl leading-7 text-slate-600">{{ $answer }}</p>
                </details>
            @endforeach
        </div>
        ...
        <div class="mt-12 rounded-2xl bg-[#08275f] p-8 text-white sm:flex sm:items-center sm:justify-between sm:gap-8">
            ...
            <a href="{{ route('contact') }}" class="mt-5 inline-flex shrink-0 items-center rounded-xl bg-white px-5 py-3 font-bold text-[#08275f] transition hover:bg-cyan-50 sm:mt-0">Contact support <i class="fas fa-arrow-right ml-2"></i></a>
        </div>
    </div>
</div>
@endsection
