...
@section('title', 'Learning History - SkillUp')
...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                    ...
                    @foreach($enrollments as $enrollment)
                        <div class="flex items-start space-x-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                            <div class="flex-shrink-0w-12 h-12 bg-gradient-to-brfrom-purple-500 to-indigo-600 rounded-xl flex items-center justify-center text-white font-bold text-sm">
                                {{ $enrollment->course->course_title[0] ?? 'C' }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <h4 class="font-semibold text-gray-900 truncate">{{ $enrollment->course->course_title }}</h4>
                                ...
                                <div class="w-full bg-gray-200 rounded-full h-2 mt-3">
                                    <div class="bg-gradient-to-r from-purple-500 to-indigo-600 h-2 rounded-full" style="width: {{ $enrollment->progress }}%"></div>
                                </div>
                            </div>
                        </div>
                    @endforeach
                    ...
                    @foreach($quizAttempts as $attempt)
                        <div class="flex items-start space-x-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                            <div class="flex-shrink-0w-12 h-12 bg-gradient-to-brfrom-green-500 to-teal-600 rounded-xl flex items-center justify-center text-white font-bold text-sm">
                                {{ $attempt->quiz->quiz_title[0] ?? 'Q' }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <h4 class="font-semibold text-gray-900 truncate">{{ $attempt->quiz->quiz_title }}</h4>
                                <div class="flex items-center justify-between text-sm text-gray-600 mt-1">
                                    <span>{{ $attempt->created_at->format('M d, Y H:i') }}</span>
                                    <span class="font-semibold text-lg">
                                        {{ number_format($attempt->score, 1) }}%
                                    </span>
                                </div>
                                @if($attempt->quiz->module && $attempt->quiz->module->course)
                                    <p class="text-xs text-gray-500 mt-1">
                                        {{ $attempt->quiz->module->course->course_title }}
                                    </p>
                                @endif
                            </div>
                        </div>
    ...
    <div class="text-center mt-12">
        <a href="{{ route('userpage.dashboard') }}" class="inline-flex items-center px-6 py-3 bg-gray-800 text-white font-semibold rounded-lg hover:bg-gray-900 transition mr-4">
