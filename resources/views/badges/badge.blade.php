...
<div class="space-y-8">
    ...
    <div class="flex items-center justify-between fade-up">
        <a href="{{ url()->previous() }}" class="inline-flex items-center px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-100 hover:-translate-x-0.5 transition-all">
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative z-10">
            ...
            <div class="flex gap-4 flex-wrap">
                <div class="stat-glass rounded-xl px-5 py-3 text-center">
                    <p class="text-2xl font-bold stat-num" data-count-target="{{ $totalEarned }}">0</p>
                    <p class="text-xs text-purple-200 uppercase tracking-wider">Total Earned</p>
                </div>
                <div class="stat-glass rounded-xl px-5 py-3 text-center">
                    <p class="text-2xl font-bold stat-num" data-count-target="{{ $moduleBadges->count() }}" style="animation-delay:.05s">0</p>
                    <p class="text-xs text-purple-200 uppercase tracking-wider">Modules</p>
                </div>
                <div class="stat-glass rounded-xl px-5 py-3 text-center">
                    <p class="text-2xl font-bold stat-num" data-count-target="{{ $quizBadges->count() }}" style="animation-delay:.1s">0</p>
                    <p class="text-xs text-purple-200 uppercase tracking-wider">Quizzes</p>
                </div>
                <div class="stat-glass rounded-xl px-5 py-3 text-center">
                    <p class="text-2xl font-bold stat-num" data-count-target="{{ $courseBadges->count() }}" style="animation-delay:.15s">0</p>
    ...
    <div>
        ...
        @if($badgeData->isEmpty())
            <div class="bg-white rounded-xl shadow-md p-12 text-center fade-up relative overflow-hidden">
                ...
                <a href="{{ route('courses.index') }}" class="explore-btn inline-flex items-center px-6 py-3 gradient-primary text-white rounded-lg shadow-md relative z-10">
                    <i class="fas fa-book-open mr-2"></i> Browse Courses
                </a>
            </div>
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($badgeData as $i => $item)
                    @php
                        $badge = $item['badge'];
                        $ub = $item['user_badge'];
                    ...
                    <div class="badge-card tilt-card neon-border bg-white rounded-xl shadow-md overflow-hidden border {{ $style['border'] }}" style="animation-delay: {{ $i * 0.06 }}s">
                        <div class="rarity-strip bg-gradient-to-r {{ $style['gradient'] }}"></div>
                        <div class="badge-shine"></div>
                        {{-- Badge Icon Header --}}
                        <div class="badge-header bg-gradient-to-r {{ $style['gradient'] }} p-6 text-center relative">
                            <div class="badge-icon-wrap badge-ring w-20 h-20 bg-white/20 rounded-full flex items-center justify-center mx-auto ring-4 {{ $style['ring'] }} backdrop-blur-sm">
    ...
    @if($unearnedBadges->isNotEmpty())
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($unearnedBadges as $i => $badge)
                    <div class="locked-card tilt-card bg-white rounded-xl shadow-md overflow-hidden border border-gray-200 opacity-95 fade-up" style="animation-delay: {{ 0.1 + ($i * 0.06) }}s">
                        ...
                        <div class="p-5">
                            ...
                            <a href="{{ route('courses.index') }}" class="explore-btn inline-flex items-center justify-center w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-100">
