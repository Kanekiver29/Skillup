...
@section('title', 'Certificate - ' . ($course->title ?? $course->course_title) . ' - SkillUp')
		...
		<div class="relative z-10 p-8 md:p-14 text-center">
			...
			<div class="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4 text-left">
				<div class="bg-white/80 border border-gray-200 rounded-xl p-4">
					...
					<p class="mt-1 text-xl font-bold text-green-700">{{ $completedModules }}/{{ $totalModules }}</p>
				</div>
				<div class="bg-white/80 border border-gray-200 rounded-xl p-4">
					<p class="text-xs uppercase text-gray-500">Completion Date</p>
					<p class="mt-1 text-xl font-bold text-gray-900">{{ $completionDate->format('M d, Y') }}</p>
				</div>
				<div class="bg-white/80 border border-gray-200 rounded-xl p-4">
					<p class="text-xs uppercase text-gray-500">Certificate ID</p>
					<p class="mt-1 text-sm sm:text-base font-bold text-gray-900 break-all">{{ $certificateCode }}</p>
				</div>
			</div>
