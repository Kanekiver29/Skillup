...
@section('title', 'Teacher Evaluation Review')
@section('page_title', 'Teacher Evaluation Review')
...
<div class="eval-show">
    ...
    <div class="es-header anim" style="animation-delay:.02s;">
        ...
        <div class="es-meta">
            ...
            <div class="es-meta-item">
                ...
                <span>{{ $evaluation->anonymous ? 'Yes, submitted anonymously' : 'No, name included' }}</span>
            </div>
            <div class="es-meta-item">
                ...
                <span style="color:var(--accent); display:inline-flex; align-items:center;">
                    ...
                    <span class="es-stars">
                        @for($i = 1; $i <= 5; $i++)
                            <svg viewBox="0 0 24 24" fill="{{ $i <= round($evaluation->rating) ? '#3b82f6' : 'none' }}" stroke="{{ $i <= round($evaluation->rating) ? '#3b82f6' : '#cbd5e1' }}" stroke-width="1.5"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                        @endfor
                    </span>
                </span>
            </div>
    ...
    @foreach($questions as $sIdx => $section)
        <div class="es-section anim" style="animation-delay:{{ 0.05 + ($sIdx * 0.04) }}s;">
            <div class="es-section-title">{{ $section['title'] }}</div>

            @foreach($section['items'] as $item)
                <div class="es-question">
                    <p class="es-q-text">{{ $item['label'] }}</p>
                    ...
                    @endphp
                    <div class="es-ans {{ $ratingClass }}">
                        <span class="val">{{ $value ?? '-' }}</span>
                        <span class="lbl-wrap">
                            <span class="lbl">{{ $lbl }}</span>
                            <span class="bar-track">
                                <span class="bar-fill" style="--fill: {{ $fillPct }}%;"></span>
                            </span>
                        </span>
                    </div>
                </div>
    ...
    <div class="es-section anim" style="animation-delay:0.2s;">
        ...
        @if($evaluation->comments)
            <div class="es-comment">{{ $evaluation->comments }}</div>
        @else
    ...
    <div class="es-actions anim" style="animation-delay:0.25s;">
        <a href="{{ route('sias.student.teacher_evaluation') }}" class="btn btn-secondary">
