...
@section('page_title', 'Teacher Evaluation')
...
<div class="eval-page">
    ...
    <div class="eval-header anim" style="animation-delay:.02s;">
        ...
        @endphp
        @if($firstPending)
            <a href="{{ route('sias.student.teacher_evaluation.create', $firstPending) }}" class="btn-action primary">
    ...
    @else
        <div class="course-grid">
            @foreach($enrollments as $index => $enrollment)
                ...
                @endphp
                <div class="course-card anim" style="animation-delay:{{ $delay }}s;">
                    @if($isDone)
                    ...
                    <h4>{{ $enrollment->course->title ?? 'Untitled Course' }}</h4>
                    <div class="prof">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                        Prof. {{ $enrollment->course->instructor->name ?? $enrollment->course->instructor_name ?? 'TBA' }}
                    </div>
                    ...
                    <div class="actions">
                        @if($isDone)
                            ...
                            <a href="{{ route('sias.student.teacher_evaluation.show', $enrollment) }}" class="btn-action secondary">Review Answers</a>
                        @else
                            <span class="status-text">Awaiting evaluation</span>
                            <a href="{{ route('sias.student.teacher_evaluation.create', $enrollment) }}" class="btn-action primary">Evaluate</a>
                        @endif
                    </div>
                </div>
            @endforeach
