...
<div class="as-page" style="display:grid;gap:1.2rem;">
    ...
    <div class="print-header" style="border-bottom:2px solid #0f172a;padding-bottom:.75rem;margin-bottom:.5rem;">
        ...
        Assessment Report &mdash; Printed {{ now()->format('F d, Y') }}
    </div>
    ...
    <div class="as-stats anim" style="animation-delay:.06s;">
        ...
        <div class="as-stat">
            ...
            <div class="value" style="color:var(--fail);">{{ $totalAttempts - $passed }}</div>
        </div>
        <div class="as-stat">
            <div class="label">Avg Score</div>
            <div class="value warn">{{ $avgScore !== null ? $avgScore . '%' : '—' }}</div>
        </div>
        <div class="as-stat">
            <div class="label">Total XP</div>
            <div class="value" style="color:var(--accent);">{{ number_format($totalXp) }}</div>
        </div>
        <div class="as-stat">
            <div class="label">In Progress</div>
            <div class="value" style="color:var(--warn);">{{ $inProgress->count() }}</div>
        </div>
    </div>
    ...
    <div class="anim" style="animation-delay:.1s;">
        ...
        <div class="as-card">
            ...
            <div class="upcoming-row">
                <div class="info">
                    <div class="title">{{ $item['quiz_title'] }}</div>
                    <div class="meta">{{ $item['module_title'] }} &middot; Started {{ $item['started_at'] }}
                        @if($item['time_limit']) &middot; {{ $item['time_limit'] }}min limit @endif
                    </div>
                </div>
                @if($item['course_slug'] && $item['module_slug'] && $item['quiz_slug'])
                <a href="{{ route('quizzes.start', [$item['course_slug'], $item['module_slug'], $item['quiz_slug']]) }}"
    ...
    <div class="anim" style="animation-delay:.14s;">
        ...
        <div class="as-card" style="overflow-x:auto;">
            ...
            <table class="as-table">
                ...
                <tbody>
                    @foreach($completedAttempts as $i => $a)
                    <tr>
                        <td style="color:var(--muted);font-size:.8rem;">{{ $i + 1 }}</td>
                        <td style="font-weight:600;">{{ $a['quiz_title'] }}</td>
                        <td style="color:var(--sub);font-size:.85rem;">{{ $a['module_title'] }}</td>
                        <td>
                            <div class="score-bar-wrap">
                                <div class="score-bar">
                                    <div class="score-bar-fill {{ $a['passed'] ? 'pass' : 'fail' }}"
                                         style="width:{{ min($a['score'],100) }}%"></div>
                                </div>
                                <span class="score-pct {{ $a['passed'] ? 'pass' : 'fail' }}">{{ $a['score'] }}%</span>
                            </div>
                        </td>
                        <td style="font-size:.85rem;">{{ $a['correct'] }}/{{ $a['total'] }}</td>
                        <td style="font-size:.82rem;color:var(--muted);">{{ $a['time_spent'] }}</td>
                        <td style="font-size:.85rem;font-weight:600;color:var(--accent);">+{{ $a['xp'] }}</td>
                        <td><span class="badge {{ $a['passed'] ? 'pass' : 'fail' }}">
                            {{ $a['passed'] ? '✓ Passed' : '✗ Failed' }}
                        </span></td>
                        <td style="font-size:.8rem;color:var(--muted);white-space:nowrap;">{{ $a['completed_at'] }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    <div class="anim" style="animation-delay:.18s;">
        ...
        <div class="as-card">
            @if($upcoming->count())
                ...
                <div class="upcoming-row">
                    <div class="info">
                        <div class="title">{{ $q['quiz_title'] }}</div>
                        <div class="meta">
                            {{ $q['module_title'] }}
                            @if($q['time_limit']) &middot; {{ $q['time_limit'] }}min @endif
                            &middot; Pass at {{ $q['passing_score'] ?? 75 }}%
                            @if($q['attempt_limit']) &middot; {{ $q['attempt_limit'] }} attempt(s) @endif
                        </div>
                    </div>
                    @if($q['course_slug'] && $q['module_slug'] && $q['quiz_slug'])
                    <a href="{{ route('quizzes.start', [$q['course_slug'], $q['module_slug'], $q['quiz_slug']]) }}"
