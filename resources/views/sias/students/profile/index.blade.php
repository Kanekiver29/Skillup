        ...
        <div class="anim-in" style="display:flex; justify-content:space-between; align-items:flex-start; gap:1rem; flex-wrap:wrap; animation-delay:.02s;">
            ...
            <div style="display:flex;align-items:center;gap:.65rem;flex-wrap:wrap;">
                <span style="display:inline-flex;align-items:center;gap:.4rem;padding:.45rem .7rem;border-radius:999px;background:var(--block-bg);border:1px solid var(--block-border);color:var(--text-muted);font-size:.78rem;font-weight:700;">
                    <i class="fa-solid fa-shield-halved" style="color:var(--accent-strong);"></i>{{ $profileVisibilityLabel }}
                </span>
        ...
        <div class="grid-2col" style="display:grid;grid-template-columns:1.2fr .8fr;gap:1rem;align-items:start;">
            <div style="display:grid;gap:1rem;">
                <div class="panel anim-in" style="animation-delay:.08s; display:grid;gap:1.25rem;">
                    <div class="grid-3col stagger" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1rem;">
                        <div style="display:grid;gap:.35rem;">
                            ...
                            <div class="field-box">{{ $courseTitle }}</div>
                        </div>
                        <div style="display:grid;gap:.35rem;">
                            <span class="field-label">Level</span>
                            <div class="field-box">Year {{ $yearLevel }}</div>
                        </div>
                        <div style="display:grid;gap:.35rem;">
                            <span class="field-label">Dept</span>
                            <div class="field-box">{{ $courseDept }}</div>
                        </div>
                    </div>
                    ...
                    <div class="grid-3col stagger" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1rem;">
                        <div style="display:grid;gap:.35rem;">
                            ...
                            <div class="field-box">{{ $curriculum }}</div>
                        </div>
                        <div style="display:grid;gap:.35rem;">
                            <span class="field-label">Year Level</span>
                            <div class="field-box">{{ $yearLevel }}</div>
                        </div>
                        <div style="display:grid;gap:.35rem;">
                            <span class="field-label">Learner Ref. No</span>
                            <div class="field-box">{{ $learnerRef }}</div>
                        </div>
                    </div>
                    ...
                    <div class="grid-3col stagger" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1rem;align-items:end;">
                        <div style="display:grid;gap:.35rem;">
                            ...
                            <div class="field-box">{{ $entryPeriod }}</div>
                        </div>
                        <div style="display:grid;gap:.35rem;">
                            <span class="field-label">Entry Date</span>
                            <div class="field-box">{{ $entryDate }}</div>
                        </div>
                        <div style="display:grid;gap:.35rem;">
                            <span class="field-label">Exam Score</span>
                            <div class="field-box">{{ $examScore }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid-3col stagger" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1rem;">
                    <div class="panel" style="display:grid;gap:.75rem;">
                        ...
                        <div class="field-box">{{ $nstpNo }}</div>
                    </div>
                    <div class="panel" style="display:grid;gap:.75rem;">
                        <span class="field-label">Pref. Modality</span>
                        <div class="field-box">{{ $prefModality }}</div>
                    </div>
                    <div class="panel" style="display:grid;gap:.75rem;">
                        <span class="field-label">Campus</span>
                        <div class="field-box">{{ $campus }}</div>
                    </div>
                </div>
                ...
                <div class="grid-2col anim-in" style="animation-delay:.18s; display:grid;grid-template-columns:1.3fr .7fr;gap:1rem;">
                    <div class="panel">
                        ...
                        <div style="display:flex;gap:1rem;flex-wrap:wrap;margin-top:.75rem;">
                            <label class="type-option">
                                <input type="checkbox" disabled {{ $type === 'New Student' ? 'checked' : '' }}>
                                New Student
                            </label>
                            <label class="type-option">
                                <input type="checkbox" disabled {{ $type === 'Regular' ? 'checked' : '' }}>
                                Regular
                            </label>
                        </div>
                    </div>
                    <div class="panel" style="display:grid;gap:.75rem;">
                        ...
                        <div class="field-box">{{ $sectionNo }}</div>
                    </div>
                </div>
                    ...
                    <details class="profile-sections-toggle" open>
                        ...
                        <div class="profile-links-wrap">
                            <div class="profile-links">
                                <a href="{{ route('sias.student.profile.personal_information') }}"><i class="fa-solid fa-user"></i>Personal Information</a>
                                <a href="{{ route('sias.student.profile.addresses_contacts') }}"><i class="fa-solid fa-map-location-dot"></i>Addresses &amp; Contacts</a>
                                <a href="{{ route('sias.student.profile.family_background') }}"><i class="fa-solid fa-people-group"></i>Family Background</a>
                                <a href="{{ route('sias.student.profile.educational_background') }}"><i class="fa-solid fa-graduation-cap"></i>Educational Background</a>
                                <a href="{{ route('sias.student.profile.religious_background') }}"><i class="fa-solid fa-heart"></i>Religious Background</a>
                                <a href="{{ route('sias.student.profile.medical_information') }}"><i class="fa-solid fa-stethoscope"></i>Medical Information</a>
                                <a href="{{ route('sias.student.profile.employment_records') }}"><i class="fa-solid fa-briefcase"></i>Employment Records</a>
                                <a href="{{ route('sias.student.profile.classifications_disabilities') }}"><i class="fa-solid fa-universal-access"></i>Classifications &amp; Disabilities</a>
                            </div>
                        </div>
                    </details>
            ...
            <aside class="panel anim-in" style="animation-delay:.3s; display:grid;gap:1rem;">
                ...
                <div style="display:grid;gap:.25rem;">
                    <div class="sidebar-row"><span style="color:var(--text-muted);">Name</span><strong>{{ auth()->user()->name ?? 'Student Name' }}</strong></div>
                    <div class="sidebar-row"><span style="color:var(--text-muted);">Email</span><strong>{{ auth()->user()->email ?? 'student@example.com' }}</strong></div>
                    <div class="sidebar-row"><span style="color:var(--text-muted);">Enrollment status</span><strong>{{ ucfirst($enrollment?->status ?? 'Active') }}</strong></div>
                    <div class="sidebar-row"><span style="color:var(--text-muted);">Enrollments</span><strong>{{ auth()->user()->enrollments()->count() }}</strong></div>
                </div>
