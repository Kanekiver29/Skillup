...
<head>
  ...
  <title>@yield('title', 'SIAS — TESDA Administration')</title>
...
<body>
  ...
  <div class="admin-shell">
    ...
    <aside id="adminSidebar" class="admin-sidebar" aria-label="Main navigation">
      ...
      <nav id="adminNav">
        ...
        @foreach ($navGroups as $group)
          @if ($group['label'])
            ...
            @endphp
            <button type="button"
                    class="nav-group-btn {{ $groupHasActive ? 'open' : '' }}"
                    data-group-btn="{{ $loop->index }}"
                    aria-expanded="{{ $groupHasActive ? 'true' : 'false' }}"
                    aria-controls="navGroupPanel{{ $loop->index }}"
                    title="{{ $group['label'] }}">
              <span class="nav-group-icon" aria-hidden="true">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">{!! $navIcons[$group['icon']] ?? '' !!}</svg>
              </span>
            ...
            <div class="nav-sub-wrap {{ $groupHasActive ? 'open' : '' }}" id="navGroupPanel{{ $loop->index }}" data-group-panel="{{ $loop->index }}">
              <div class="nav-sub-inner">
                <div class="nav-sub">
                  @foreach ($group['items'] as $item)
                    <a href="{{ route($item['route'], $item['params'] ?? []) }}" class="{{ $isItemActive($item) ? 'active' : '' }}">
                ...
                <a href="{{ route($item['route'], $item['params'] ?? []) }}"
                  ...
                  @if (isset($item['icon']))
                    <span class="nav-item-icon" aria-hidden="true">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">{!! $navIcons[$item['icon']] ?? '' !!}</svg>
                    </span>
                  @endif
                  <span>{{ $item['label'] }}</span>
                </a>
      ...
      <div class="sidebar-actions">
        <a href="{{ route('sias.admin.students.index') }}" class="btn" title="View all students">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
        ...
        <a href="{{ route('sias.admin.students.create') }}" class="btn btn-secondary" title="Register a new student">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true">
        ...
        <form method="POST" action="{{ route('logout') }}" style="width:100%;margin:0;" data-confirm-submit="Sign out of the admin panel?">
          @csrf
    ...
    <main id="adminContent" class="admin-content">
      <header class="admin-header">
        <div class="admin-header-title">
          ...
          <div>
            <h2>@yield('page_title', 'SIAS Admin')</h2>
            @hasSection('subtitle')
              <p>@yield('subtitle')</p>
            @endif
          </div>
        ...
        <div class="admin-header-actions">
          ...
          @auth
            <a href="{{ route('notifications.index') }}" id="adminBell" class="icon-btn topbar-bell" aria-label="Notifications">
          ...
          <div class="lang-switch" role="group" aria-label="Language">
            @foreach (['en' => 'EN', 'tl' => 'TL'] as $code => $label)
              <form method="GET" action="{{ route('language.switch', $code) }}" class="lang-form">
                <button type="submit"
                        class="lang-option {{ app()->getLocale() === $code ? 'active' : '' }}"
                        aria-pressed="{{ app()->getLocale() === $code ? 'true' : 'false' }}">{{ $label }}</button>
              </form>
            @endforeach
          </div>
      ...
      <div id="adminMain">@yield('content')</div>
    </main>
  </div>
  ...
  <script>
    (function () {
      ...
      if (badge && bell) {
        var notifUrl = @json(route('chats.notifications'));
