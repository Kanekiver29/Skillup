...
<div class="page-card" style="padding:1.5rem;">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:1rem;margin-bottom:2rem;flex-wrap:wrap;">
        ...
        <a href="{{ route('sias.admin.course.add') }}" style="display:inline-flex;align-items:center;gap:.6rem;padding:.85rem 1.2rem;background:#1d4ed8;color:#fff;border-radius:14px;text-decoration:none;font-weight:700;transition:all .2s ease;"><i class="fa-solid fa-plus"></i> <span data-i18n="add_course">Add Course</span></a>
    </div>
    ...
    <div style="display:grid;gap:1.5rem;">
        @forelse($courses as $course)
            <div style="background:var(--block-bg);border:1px solid var(--block-border);border-radius:16px;padding:1.5rem;transition:all .2s ease;">
                <div style="display:flex;justify-content:space-between;align-items:start;gap:1rem;flex-wrap:wrap;margin-bottom:1rem;">
                    <div>
                        <h3 style="margin:0 0 .35rem;font-size:1.25rem;font-weight:700;">{{ $course->title }}</h3>
                        <p style="margin:0 0 .5rem;color:var(--text-muted);font-size:.9rem;">{{ $course->description }}</p>
                        <div style="display:flex;gap:1rem;margin-top:.75rem;flex-wrap:wrap;">
                            <span style="display:inline-flex;align-items:center;gap:.4rem;font-size:.85rem;color:var(--text-muted);"><i class="fa-solid fa-code"></i> {{ $course->code }}</span>
                            <span style="display:inline-flex;align-items:center;gap:.4rem;font-size:.85rem;color:var(--text-muted);"><i class="fa-solid fa-building"></i> {{ $course->department }}</span>
                            <span style="display:inline-flex;align-items:center;gap:.4rem;font-size:.85rem;color:var(--text-muted);"><i class="fa-solid fa-book"></i> {{ $course->subjects_count ?? 0 }} Subjects</span>
                        </div>
                    </div>
                    <div style="display:flex;gap:.5rem;">
                        <a href="{{ route('sias.admin.reports', ['department' => $course->department ?? $course->category ?? 'General', 'course_id' => $course->id]) }}" style="display:inline-flex;align-items:center;justify-content:center;width:40px;height:40px;border-radius:10px;background:rgba(16,185,129,.12);color:#059669;text-decoration:none;transition:all .2s ease;" title="Course report"><i class="fa-solid fa-chart-column"></i></a>
                        <a href="{{ route('sias.admin.course.edit', $course->id) }}" style="display:inline-flex;align-items:center;justify-content:center;width:40px;height:40px;border-radius:10px;background:var(--block-bg-hover);color:#1d4ed8;text-decoration:none;transition:all .2s ease;" title="Edit"><i class="fa-solid fa-pen"></i></a>
                ...
                <details class="form-section-toggle" style="list-style:none;margin-top:1rem;">
                    <summary style="display:flex;align-items:center;gap:.75rem;padding:1rem 1.25rem;background:rgba(29,78,216,.08);border:1px solid rgba(29,78,216,.2);border-radius:10px;cursor:pointer;user-select:none;transition:all .2s ease;font-weight:600;font-size:.95rem;margin:0;">
                        ...
                        <span>Manage Subjects ({{ $course->subjects_count ?? 0 }})</span>
                    ...
                    <div style="padding:1.5rem;border-top:1px solid var(--divider);animation:slideDown .3s ease forwards;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;gap:1rem;">
                            ...
                            <a href="{{ route('sias.admin.subject.add', $course->id) }}" style="display:inline-flex;align-items:center;gap:.4rem;padding:.6rem .9rem;background:#1d4ed8;color:#fff;border-radius:8px;text-decoration:none;font-weight:600;font-size:.85rem;transition:all .2s ease;"><i class="fa-solid fa-plus"></i> Add Subject</a>
                        </div>
                                ...
                                @foreach($course->subjects as $subject)
                                    <div style="display:flex;justify-content:space-between;align-items:center;padding:.75rem 1rem;background:var(--block-bg);border:1px solid var(--block-border);border-radius:10px;gap:1rem;flex-wrap:wrap;">
                                        <div style="flex:1;min-width:200px;">
                                            <div style="font-weight:600;color:var(--text);">{{ $subject->code }} - {{ $subject->title }}</div>
                                            <div style="font-size:.85rem;color:var(--text-muted);margin-top:.25rem;">{{ $subject->units ?? 3 }} Units</div>
                                        </div>
                                        <div style="display:flex;gap:.5rem;">
        ...
        @empty
            <div style="text-align:center;padding:3rem 1rem;background:var(--block-bg);border:1px dashed var(--block-border);border-radius:16px;">
                ...
                <a href="{{ route('sias.admin.course.add') }}" style="display:inline-flex;align-items:center;gap:.6rem;padding:.85rem 1.2rem;background:#1d4ed8;color:#fff;border-radius:14px;text-decoration:none;font-weight:700;transition:all .2s ease;"><i class="fa-solid fa-plus"></i> Create Course</a>
            </div>
        @endforelse
    </div>
</div>
