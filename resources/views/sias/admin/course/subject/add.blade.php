...
<div class="page-card" style="padding:1.5rem;">
    ...
    <form method="POST" action="{{ route('sias.admin.subject.store') }}" class="section-card" style="display:grid;gap:1.5rem;max-width:800px;">
        @csrf
        ...
        <details class="form-section-toggle" open>
            ...
            <div style="padding:1.5rem;display:grid;gap:1rem;animation:slideDown .3s var(--ease) forwards;">
                <div style="display:grid;gap:.35rem;">
                    ...
                    <select name="course_id" class="form-input" required>
                        ...
                        @foreach($courses as $course)
                            <option value="{{ $course->id }}" {{ old('course_id') == $course->id ? 'selected' : '' }}>{{ $course->title }} ({{ $course->code }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div style="display:grid;gap:.35rem;">
                    ...
                    <input name="title" type="text" class="form-input" placeholder="" data-i18n-placeholder="e_g_introduction_programming" value="{{ old('title') }}" required>
                </div>
                ...
                <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem;">
                    <div style="display:grid;gap:.35rem;">
                        ...
                        <input name="code" type="text" class="form-input" placeholder="" data-i18n-placeholder="e_g_cs101" value="{{ old('code') }}" required>
                    </div>
                    <div style="display:grid;gap:.35rem;">
                        <label style="font-weight:600;color:var(--text);">
                            <span data-i18n="credit_units">Credit Units</span> <span style="color:#dc2626;">*</span>
                        </label>
                        <input name="units" type="number" class="form-input" placeholder="" data-i18n-placeholder="e_g_3" min="1" max="6" value="{{ old('units', 3) }}" required>
                    </div>
                </div>

                <div style="display:grid;gap:.35rem;">
                    <label style="font-weight:600;color:var(--text);" data-i18n="description">Description</label>
                    <textarea name="description" rows="4" class="form-input" placeholder="" data-i18n-placeholder="subject_description_objectives">{{ old('description') }}</textarea>
                </div>
            </div>
        </details>
            ...
            <div style="padding:1.5rem;display:grid;gap:1rem;animation:slideDown .3s var(--ease) forwards;">
                <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1rem;">
                    ...
                    <div style="display:grid;gap:.35rem;">
                        ...
                        <input name="lecture_hours" type="number" class="form-input" placeholder="e.g. 3" min="0" value="{{ old('lecture_hours', 3) }}">
                    </div>
                </div>
                ...
                <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem;">
                    <div style="display:grid;gap:.35rem;">
                        ...
                        <input name="lab_hours" type="number" class="form-input" placeholder="e.g. 2" min="0" value="{{ old('lab_hours', 0) }}">
                    </div>
                    <div style="display:grid;gap:.35rem;">
                        <label style="font-weight:600;color:var(--text);">
                            <input type="checkbox" name="is_required" value="1" {{ old('is_required') ? 'checked' : '' }} style="width:1rem;height:1rem;margin-right:.5rem;cursor:pointer;">
                            Required Subject
                        </label>
                    </div>
                </div>
