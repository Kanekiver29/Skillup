...
@section('title', 'Backup & Restore | SIAS Admin')
@section('page_title', 'Backup & Restore')
@section('subtitle', 'Create, download, and safely restore SIAS database and application backups.')
...
<div class="admin-grid" style="grid-template-columns:repeat(auto-fit, minmax(180px, 1fr)); margin-bottom:1rem;">
    <div class="admin-panel backup-stat"><span style="color:var(--text-muted);">Database</span><strong style="display:block; margin-top:.35rem;">{{ strtoupper($driver) }}</strong><small style="color:var(--text-muted);">{{ $databaseName }}</small></div>
    <div class="admin-panel backup-stat"><span style="color:var(--text-muted);">Backup files</span><strong style="display:block; font-size:1.7rem; margin-top:.3rem;">{{ $backupFiles->count() }}</strong></div>
    <div class="admin-panel backup-stat"><span style="color:var(--text-muted);">Storage</span><strong style="display:block; margin-top:.35rem; color:{{ $diagnostics['backup_dir_writable'] ? '#15803d' : 'var(--danger)' }};">{{ $diagnostics['backup_dir_writable'] ? 'Writable' : 'Not writable' }}</strong></div>
</div>
...
<div class="admin-grid" style="grid-template-columns:minmax(0, 2fr) minmax(260px, 1fr); align-items:start;">
    <section class="admin-card" style="padding:0; overflow:hidden;">
        <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:1rem; flex-wrap:wrap; padding:1.2rem 1.25rem; border-bottom:1px solid var(--card-border);">
            ...
            <div class="backup-actions">
                <form method="POST" action="{{ route('admin.backup.store') }}">@csrf<button class="btn" type="submit" style="width:auto;">Database backup</button></form>
                <form method="POST" action="{{ route('sias.admin.backup.whole-system') }}">@csrf<input type="hidden" name="include_all" value="1"><button class="btn backup-primary" type="submit" style="width:auto;">Whole system backup</button></form>
            </div>
        </div>
            ...
            <table class="backup-table" style="width:100%; border-collapse:collapse; min-width:800px;">
                ...
                @forelse($backupFiles as $file)
                    <tr style="border-bottom:1px solid var(--card-border); vertical-align:top;">
                        <td style="padding:.85rem 1rem; font-weight:600; overflow-wrap:anywhere;">{{ $file['name'] }}</td>
                        <td style="padding:.85rem 1rem;"><span style="padding:.25rem .5rem; border-radius:999px; font-size:.72rem; background:{{ $file['type'] === 'Full Backup' ? 'rgba(34,197,94,.12)' : 'var(--accent-tint)' }}; color:{{ $file['type'] === 'Full Backup' ? '#15803d' : 'var(--accent)' }};">{{ $file['type'] }}</span></td>
                        <td style="padding:.85rem 1rem; color:var(--text-muted);">{{ $file['size'] >= 1048576 ? number_format($file['size'] / 1048576, 2) . ' MB' : number_format($file['size'] / 1024, 2) . ' KB' }}</td>
                        <td style="padding:.85rem 1rem; white-space:nowrap; color:var(--text-muted);">{{ \Carbon\Carbon::createFromTimestamp($file['modified'])->format('M d, Y H:i') }}</td>
                        <td style="padding:.85rem 1rem;">
                            <div style="display:flex; gap:.4rem; flex-wrap:wrap;">
                                <a class="btn btn-secondary" style="width:auto; padding:.55rem .7rem; font-size:.78rem;" href="{{ route('admin.backup.download', urlencode($file['name'])) }}">Download</a>
                                <form method="POST" action="{{ route('admin.backup.restore') }}" onsubmit="return confirm('Restore {{ $file['name'] }}? This may overwrite current data.');">@csrf<input type="hidden" name="backup_file" value="{{ $file['name'] }}">@if($file['type'] === 'Full Backup')<input type="hidden" name="confirm" value="yes">@endif<button class="btn btn-secondary" style="width:auto; padding:.55rem .7rem; font-size:.78rem; color:var(--danger);" type="submit">Restore</button></form>
                            </div>
                        </td>
                    </tr>
                @empty
    ...
    <aside class="admin-card backup-diagnostics">
        ...
        <div style="display:grid; gap:.65rem; font-size:.85rem;">
            <div><strong>ZIP:</strong> {{ $diagnostics['zip'] ? 'Available' : 'Missing' }}</div>
            <div><strong>PharData:</strong> {{ $diagnostics['phar'] ? 'Available' : 'Missing' }}</div>
            <div><strong>Backup directory:</strong><br><span style="color:var(--text-muted); overflow-wrap:anywhere;">{{ $diagnostics['backup_dir'] }}</span></div>
            <div><strong>Writable:</strong> {{ $diagnostics['backup_dir_writable'] ? 'Yes' : 'No' }}</div>
        </div>
        ...
        @if($diagnostics['log_tail'])<details style="margin-top:1rem;"><summary style="cursor:pointer; font-weight:600;">Recent backup log</summary><pre style="white-space:pre-wrap; max-height:220px; overflow:auto; font-size:.72rem; color:var(--text-muted);">{{ $diagnostics['log_tail'] }}</pre></details>@endif
    </aside>
</div>
</div>
@endsection
