...
@section('title', __('sias.teacher_settings'))
@section('page_title', __('sias.teacher_settings'))
...
<div class="teacher-settings page-card">
    <div class="settings-hero">
        <div>
            <p class="settings-kicker">{{ __('sias.faculty_workspace') }}</p>
            <h2>{{ __('sias.teacher_settings') }}</h2>
            <p>{{ __('sias.update_teacher_preferences') }}</p>
        </div>
        <div class="settings-status"><i class="fa-solid fa-circle-check"></i> {{ __('sias.account_active') }}</div>
        <a class="btn" href="{{ route('sias.teacher.account.password') }}" aria-label="Change password">
    ...
    <form method="POST" action="{{ route('sias.teacher.settings.update') }}" class="settings-form">
        @csrf

        <fieldset class="settings-section">
            <legend><i class="fa-solid fa-user"></i> {{ __('sias.account') }}</legend>
            <div class="settings-grid">
                <label class="settings-field">{{ __('sias.name') }}
                    <input type="text" name="name" value="{{ old('name', $user->name) }}" required>
                </label>
                <label class="settings-field">{{ __('sias.email') }}
                    <input type="email" name="email" value="{{ old('email', $user->email) }}" required>
                </label>
                <label class="settings-field">{{ __('sias.phone') }}
                    <input type="tel" name="account[phone]" value="{{ old('account.phone', $account['phone'] ?? '') }}">
                </label>
                <label class="settings-field">{{ __('sias.language') }}
                    <select name="account[language]">
                        @foreach (['English', 'Filipino'] as $language)
                            <option value="{{ $language }}" @selected(old('account.language', $account['language'] ?? $activeLanguage) === $language)>{{ $language }}</option>
                        @endforeach
                    </select>
                </label>
            </div>
        ...
        <fieldset class="settings-section">
            <legend><i class="fa-solid fa-sliders"></i> {{ __('sias.appearance_notifications') }}</legend>
            <div class="settings-grid">
                <label class="settings-field">{{ __('sias.theme') }}
                    <select id="appearance_theme" name="appearance[theme]">
                        @foreach (['light' => 'Light', 'dark' => 'Dark', 'system' => 'System'] as $value => $label)
                            <option value="{{ $value }}" @selected(old('appearance.theme', $appearance['theme'] ?? 'light') === $value)>{{ __('sias.' . $value) }}</option>
                        @endforeach
                    </select>
                </label>
                <label class="settings-field">{{ __('sias.font_size') }}
                    <select name="appearance[font_size]">
                        @foreach (['small', 'medium', 'large'] as $value)
                            <option value="{{ $value }}" @selected(old('appearance.font_size', $appearance['font_size'] ?? 'medium') === $value)>{{ __('sias.' . $value) }}</option>
                        @endforeach
                    </select>
                </label>
                <label class="settings-toggle">
                    <input type="checkbox" name="appearance[reduced_motion]" value="1" @checked(old('appearance.reduced_motion', $appearance['reduced_motion'] ?? false))>
                    {{ __('sias.reduce_animations') }}
                </label>
                <label class="settings-toggle">
                    <input type="checkbox" name="notifications[email]" value="1" @checked(old('notifications.email', $notifications['email'] ?? true))>
                    {{ __('sias.email_notifications') }}
                </label>
                <label class="settings-toggle">
                    <input type="checkbox" name="notifications[push]" value="1" @checked(old('notifications.push', $notifications['push'] ?? true))>
                    {{ __('sias.push_notifications') }}
                </label>
                <label class="settings-toggle">
                    <input type="checkbox" name="notifications[weekly_digest]" value="1" @checked(old('notifications.weekly_digest', $notifications['weekly_digest'] ?? false))>
                    {{ __('sias.weekly_digest') }}
                </label>
            </div>
        </fieldset>

        <fieldset class="settings-section">
            <legend><i class="fa-solid fa-shield-halved"></i> {{ __('sias.privacy') }}</legend>
            <div class="settings-grid">
                <label class="settings-field">{{ __('sias.profile_visibility') }}
                    <select name="privacy[profile_visibility]">
                        @foreach (['public', 'students_only', 'private'] as $value)
                            <option value="{{ $value }}" @selected(old('privacy.profile_visibility', $privacy['profile_visibility'] ?? 'public') === $value)>{{ __('sias.' . $value) }}</option>
                        @endforeach
                    </select>
                </label>
                <label class="settings-toggle">
                    <input type="checkbox" name="privacy[show_contact]" value="1" @checked(old('privacy.show_contact', $privacy['show_contact'] ?? false))>
                    {{ __('sias.show_contact') }}
                </label>
            </div>
        </fieldset>
