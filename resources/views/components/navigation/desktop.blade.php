...
<header id="site-header" class="site-header fixed inset-x-0 top-0 z-50 text-slate-100">
    ...
    <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" aria-label="Main navigation">
        <div class="site-header-bar flex items-center justify-between gap-4 lg:gap-6">
            <a href="{{ route('home') }}" class="header-brand inline-flex items-center gap-3 sm:gap-4 text-white shrink-0" aria-label="SkillUp Home">
            ...
            @guest
                <div class="hidden lg:flex flex-1 justify-center">
                    <div id="primary-nav" class="font-display flex items-center gap-8 xl:gap-10 text-[11px] font-semibold uppercase tracking-[0.14em]">
                        @foreach ($publicItems as $item)
                            ...
                            @endphp
                            <a href="{{ $item['route'] }}" class="nav-link {{ $isActive ? 'active' : '' }}" title="{{ $item['label'] }}">
                                {{ $item['label'] }}
                            </a>
                        @endforeach
                    </div>
                ...
                <div class="hidden lg:flex items-center gap-2.5 shrink-0">
                    <form action="{{ route('courses.index') }}" method="GET" class="header-search relative flex items-center rounded-full border border-slate-600/60 bg-slate-950/30 px-3 py-2 w-[14rem] xl:w-[20rem]" aria-label="Search courses">
                    ...
                    <a href="{{ route('login') }}" class="header-action-btn header-login-btn btn-ripple inline-flex items-center justify-center rounded-full px-5 py-2.5 text-sm font-semibold text-white">Login</a>
                    <a href="{{ route('register') }}" class="header-action-btn header-signup-btn btn-ripple inline-flex items-center justify-center rounded-full px-5 py-2.5 text-sm font-bold shadow-md">Sign Up</a>
                </div>
            @else
                <div class="hidden lg:flex flex-1 justify-center">
                    <div id="primary-nav" class="font-display flex items-center gap-8 xl:gap-10 text-[11px] font-semibold uppercase tracking-[0.14em]">
                ...
                <div class="hidden lg:flex items-center gap-2.5 shrink-0">
                    <form action="{{ route('courses.index') }}" method="GET" class="header-search relative flex items-center rounded-full border border-slate-600/60 bg-slate-950/30 px-3 py-2 w-[14rem] xl:w-[18rem]" aria-label="Search courses">
                        <i class="header-search-icon fas fa-search text-slate-400 text-sm" aria-hidden="true"></i>
