...
@section('page_title', 'Grade Sheet')
...
<div class="grade-sheet-wrap" style="max-width:1280px;margin:0 auto;padding:2rem 1.25rem 3rem;">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:1rem;flex-wrap:wrap;margin-bottom:1.5rem;">
        ...
        <div style="display:flex;gap:.75rem;flex-wrap:wrap;">
            ...
            <a href="{{ route('teacher.grades.print') }}" target="_blank" rel="noopener" style="padding:.75rem 1rem;border-radius:.8rem;background:#0f766e;color:#fff;font-weight:700;">Print Grades</a>
            <a href="{{ route('teacher.grades.reports') }}" style="padding:.75rem 1rem;border-radius:.8rem;background:#edf2ff;color:#182c63;font-weight:700;">View Reports</a>
        </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:.85rem;margin-bottom:1.5rem;">
        @foreach([
            ...
            <div style="background:#fff;border:1px solid #e4eaf7;border-radius:1rem;padding:1rem;box-shadow:0 12px 30px -20px rgba(9,20,51,.24);">
                <small style="display:block;color:#64768f;font-weight:700;letter-spacing:.06em;text-transform:uppercase;">{{ $stat[0] }}</small>
                <strong style="display:block;margin-top:.45rem;font-size:1.6rem;color:#0b1730;">{{ $stat[1] }}</strong>
            </div>
        @endforeach
    </div>
    ...
    <div id="grade-add-form" class="grade-form-shell">
        ...
        <form method="POST" action="{{ route('teacher.grades.store') }}" class="grade-form-row">
            @csrf
            <div class="grade-form-field">
                ...
                <select id="student_id" name="student_id" required>
                    ...
                    @foreach($enrollments as $enrollment)
                        <option value="{{ $enrollment->user_id }}">{{ $enrollment->user?->name }} · {{ $enrollment->course?->title }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <table class="grade-sheet-table">
            ...
            <tbody>
                @forelse($manualGrades as $grade)
                    <tr>
                        <td style="font-weight:700;color:#0b1730;">{{ $grade->student?->name ?? 'Student' }}</td>
                        <td style="color:#33415c;">{{ $grade->course?->title ?? '—' }}</td>
                        <td style="color:#33415c;">
                            <div style="font-weight:800;">{{ $grade->title }}</div>
                            <div class="grade-badge" style="margin-top:.35rem;">{{ ucfirst($grade->assessment_type) }}</div>
                        </td>
                        <td>
                            <form method="POST" action="{{ route('teacher.grades.update', $grade) }}">
                                @csrf
                                @method('PATCH')
                                <input name="score" type="number" min="0" step="0.01" value="{{ $grade->score }}" required class="cell-input cell-input--small">
                        </td>
                        <td>
                                <input name="max_score" type="number" min="0.01" step="0.01" value="{{ $grade->max_score }}" required class="cell-input cell-input--small">
                        </td>
                        <td class="grade-percent">
                                {{ number_format($grade->percentage, 1) }}%
                        </td>
                        <td>
                                <input name="remarks" value="{{ $grade->remarks }}" placeholder="Remark" class="cell-input">
                        </td>
                        <td>
                ...
                @foreach($enrollments as $enrollment)
                    ...
                    @if(! $hasGrade)
                        <tr>
                            <td style="font-weight:700;color:#0b1730;">{{ $enrollment->user?->name ?? 'Student' }}</td>
                            <td style="color:#33415c;">{{ $enrollment->course?->title ?? '—' }}</td>
                            <td>
