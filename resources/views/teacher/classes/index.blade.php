...
@section('subtitle', 'Batches, trainees, training status and schedules')
...
<div class="classes-page">
  ...
  <section class="class-summary" aria-label="Portfolio summary">
    <div class="summary-item">
      ...
      <span class="summary-label">Assigned {{ \Illuminate\Support\Str::plural('batch', $items->count()) }}</span>
    </div>
    <div class="summary-item">
      <span class="summary-value">{{ number_format($totalTrainees) }}</span>
      <span class="summary-label">Trainees enrolled</span>
    </div>
    <div class="summary-item">
      <span class="summary-value">{{ $statusCounts['ongoing'] ?? 0 }}</span>
      <span class="summary-label">Ongoing</span>
    </div>
    <div class="summary-item">
      <span class="summary-value">{{ $statusCounts['upcoming'] ?? 0 }}</span>
      <span class="summary-label">Not yet started</span>
    </div>
    @if ($capacityTotal > 0)
      <div class="summary-item">
        <span class="summary-value">{{ $capacityTotal - $totalTrainees }}</span>
  ...
  <section class="class-toolbar" aria-label="Filter classes">
    ...
    <div class="filter-chips" role="group" aria-label="Filter by status">
      @foreach (['all' => 'All', 'ongoing' => 'Ongoing', 'upcoming' => 'Upcoming', 'completed' => 'Completed'] as $key => $label)
        <button type="button" class="chip {{ $key === 'all' ? 'is-active' : '' }}" data-filter="{{ $key }}" aria-pressed="{{ $key === 'all' ? 'true' : 'false' }}">
          {{ $label }}
        </button>
      @endforeach
    </div>
  ...
  <div class="class-grid" id="classGrid">
    @forelse ($items as $course)
      ...
      <article class="class-card"
               ...
               data-trainees="{{ $enrolled }}"
               data-start="{{ $sortStart }}"
        ...
        <header class="card-head">
          ...
          <span class="status-tag status-{{ $status }}">{{ $statusLabels[$status] ?? ucfirst($status) }}</span>
        </header>

        <dl class="card-facts">
          <div>
            <dt>Trainees</dt>
            <dd>{{ $enrolled }}@if ($capacity > 0)<span class="fact-muted"> / {{ $capacity }}</span>@endif</dd>
          </div>
          @if ($start || $end)
            <div>
              <dt>Training period</dt>
              <dd>
                {{ $start ? \Illuminate\Support\Carbon::parse($start)->format('d M Y') : '—' }}
                &ndash;
                {{ $end ? \Illuminate\Support\Carbon::parse($end)->format('d M Y') : '—' }}
              </dd>
            </div>
          @endif
          @if ($hours)
        ...
        @if ($capacity > 0)
          @php $fill = (int) min(100, round($enrolled / max(1, $capacity) * 100)); @endphp
          <div class="meter" role="img" aria-label="{{ $fill }} percent of slots filled">
            <span class="meter-fill" style="width: {{ $fill }}%"></span>
          </div>
          <p class="meter-note">{{ max(0, $capacity - $enrolled) }} slots remaining</p>
        @elseif (!is_null($progress))
          <div class="meter" role="img" aria-label="{{ $progress }} percent of the training period elapsed">
            <span class="meter-fill" style="width: {{ $progress }}%"></span>
          </div>
          <p class="meter-note">{{ $progress }}% of the training period elapsed</p>
        @endif
        ...
        @if ($schedules->isNotEmpty())
          <ul class="schedule-list">
            @foreach ($schedules->take(3) as $schedule)
              ...
              @endphp
              <li>
                <span class="schedule-day">{{ $day ?? 'Scheduled' }}</span>
                @if ($from || $to)
                  <span class="schedule-time">
                    {{ $from ? \Illuminate\Support\Carbon::parse($from)->format('g:i A') : '' }}@if ($from && $to) – @endif{{ $to ? \Illuminate\Support\Carbon::parse($to)->format('g:i A') : '' }}
                  </span>
                @endif
              </li>
            @endforeach
            @if ($schedules->count() > 3)
              <li class="schedule-more">+{{ $schedules->count() - 3 }} more sessions</li>
            @endif
          </ul>
        @endif
        ...
        @if ($subjects->isNotEmpty())
          <p class="subject-line">
            {{ $subjects->count() }} {{ \Illuminate\Support\Str::plural('subject', $subjects->count()) }}:
            {{ $subjects->take(3)->map(fn ($s) => $s->title ?? $s->name)->filter()->implode(', ') }}@if ($subjects->count() > 3)…@endif
          </p>
        @endif

        <footer class="card-actions">
          <a class="action action-primary" href="{{ route('teacher.courses.show', $course) }}">View program</a>
          @if (\Illuminate\Support\Facades\Route::has('teacher.courses.trainees'))
            <a class="action" href="{{ route('teacher.courses.trainees', $course) }}">Trainees</a>
          @endif
          @if (\Illuminate\Support\Facades\Route::has('teacher.attendance.index'))
            <a class="action" href="{{ route('teacher.attendance.index', ['course' => $course->getKey()]) }}">Attendance</a>
          @endif
          @if (\Illuminate\Support\Facades\Route::has('teacher.grades.index'))
            <a class="action" href="{{ route('teacher.grades.index', ['course' => $course->getKey()]) }}">Grades</a>
          @endif
        </footer>
      </article>
    @empty
  ...
  {{-- Shown by JS when search/filter clears the grid --}}
  ...
  @if ($courses instanceof \Illuminate\Contracts\Pagination\Paginator
      || $courses instanceof \Illuminate\Pagination\AbstractPaginator)
    <div class="class-pagination">{{ $courses->withQueryString()->links() }}</div>
  @endif
</div>
