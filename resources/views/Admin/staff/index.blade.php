...
<div class="sm-page">
    <header class="sm-header">
        ...
        <div class="sm-header-actions">
            <a href="{{ route('admin.staff.create') }}" class="sm-btn sm-btn-emerald">
                <i class="fas fa-user-plus"></i> Create Staff
            </a>
            <a href="{{ route('admin.staff.register') }}" class="sm-btn sm-btn-blue">
    ...
    <div class="sm-stats-grid">
        <div class="sm-stat-card sm-stat-primary">
            ...
            <p class="sm-stat-value">{{ number_format($totalStaff ?? 0) }}</p>
        </div>
        @foreach($staffByType as $key => $info)
            <div class="sm-stat-card">
                <p class="sm-stat-label">{{ $info['label'] }}</p>
                <p class="sm-stat-value">{{ number_format($info['count'] ?? 0) }}</p>
            </div>
        @endforeach
    </div>

    <div class="sm-card sm-filter-card">
        <form method="GET" action="{{ route('admin.staff.index') }}" class="sm-filter-form">
            ...
            <div class="sm-filter-buttons">
                ...
                <a href="{{ route('admin.staff.index') }}" class="sm-btn sm-btn-outline">
    ...
    <div class="sm-card sm-table-card">
        <div class="sm-table-header">
            ...
            <span class="sm-table-count">{{ $staff->total() }} {{ Str::plural('member', $staff->total()) }}</span>
        </div>
        <table class="sm-table">
            ...
            <tbody>
                @forelse($staff as $user)
                    <tr class="sm-row" style="animation-delay: {{ 60 + ($loop->index * 35) }}ms;">
                        <td class="sm-td-name">
                            <div class="sm-avatar">{{ strtoupper(substr($user->name, 0, 1)) }}</div>
                            <span>{{ $user->name }}</span>
                        </td>
                        <td class="sm-td-email">{{ $user->email }}</td>
                        <td>
                            <span class="sm-type-badge">{{ $staffTypes[$user->staff_type]['label'] ?? '—' }}</span>
                        </td>
                        <td class="sm-td-actions">
                            <a href="{{ route('admin.staff.transfer', $user) }}" class="sm-link-transfer">
                                <i class="fas fa-right-left"></i> Transfer
                            </a>
                            <form action="{{ route('admin.staff.demote', $user) }}" method="POST" class="sm-inline-form" onsubmit="return confirm('Demote this staff member to student?');">
                                @csrf
                                @method('DELETE')
        ...
        <div class="sm-pagination-wrap">{{ $staff->links() }}</div>
    </div>
</div>
