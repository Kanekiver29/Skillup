...
<div class="am-page">
    <header class="am-header">
        ...
        <a href="{{ route('admin.modules.create', request('course_id') ? ['course_id' => request('course_id')] : []) }}" class="am-btn-new">
    ...
    <div class="am-card am-filter-card">
        <form method="GET" action="{{ route('admin.modules.index') }}" class="am-filter-form">
            ...
            @if(request('course_id'))
                <a href="{{ route('admin.modules.index') }}" class="am-link-clear">
        ...
        <table class="am-table">
            ...
            <tbody>
                @forelse($modules as $module)
                    <tr class="am-row" style="animation-delay: {{ 80 + ($loop->index * 40) }}ms;">
                        <td>
                            <span class="am-order-badge">{{ $module->order }}</span>
                        </td>
                        <td>
                            <div class="am-module-title">{{ $module->title }}</div>
                            @if($module->description)
                                <p class="am-module-desc">{{ $module->description }}</p>
                            @endif
                        </td>
                        <td>
                            <span class="am-tag am-tag-blue">{{ $module->course->title ?? 'N/A' }}</span>
                        </td>
                        <td>
                            <span class="am-tag am-tag-purple">
                                {{ $module->quizzes_count }} {{ Str::plural('quiz', $module->quizzes_count) }}
                            </span>
                        </td>
                        <td>
                        ...
                        <td class="am-td-actions">
                            <a href="{{ route('admin.modules.edit', $module) }}" class="am-link-edit">
                                <i class="fas fa-edit"></i> Edit
                            </a>
                            <form action="{{ route('admin.modules.destroy', $module) }}" method="POST" class="am-inline-form" onsubmit="return confirm('Archive module \'{{ $module->title }}\'? It can be restored later.');">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="6" class="am-empty-state">
                            ...
                            <a href="{{ route('admin.modules.create') }}" class="am-link-empty-cta">
    ...
    <div class="am-archived-section">
        ...
        <div class="am-card am-table-card am-archived-card">
            <table class="am-table">
                ...
                <tbody>
                    @foreach($archivedModules as $archived)
                        <tr class="am-row">
                            <td class="am-archived-title-cell">{{ $archived->title }}</td>
                            <td>
                                <span class="am-tag am-tag-blue">{{ $archived->course->title ?? 'N/A' }}</span>
                            </td>
                            <td class="am-archived-date">{{ $archived->deleted_at->format('M d, Y') }}</td>
                            <td class="am-td-actions">
                                <form action="{{ route('admin.modules.restore', $archived->id) }}" method="POST" class="am-inline-form" onsubmit="return confirm('Restore module \'{{ $archived->title }}\'?');">
                                    @csrf
                                    <button type="submit" class="am-link-restore">
                                        <i class="fas fa-undo"></i> Restore
                                    </button>
                                </form>
                                <form action="{{ route('admin.modules.force-delete', $archived->id) }}" method="POST" class="am-inline-form" onsubmit="return confirm('Permanently delete module \'{{ $archived->title }}\'? This cannot be undone and will remove all quizzes, questions, and student attempts.');">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="am-footer-actions">
        <a href="{{ route('admin.courses.index') }}" class="am-btn-secondary am-btn-blue">
            View Courses
        </a>
        <a href="{{ route('admin.dashboard') }}" class="am-btn-secondary am-btn-gray">
            Back to Dashboard
        </a>
    </div>
</div>
