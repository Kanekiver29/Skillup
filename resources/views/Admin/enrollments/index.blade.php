        ...
        <table class="en-table">
            ...
            <tbody>
                @forelse($enrollments as $e)
                    <tr class="en-row" style="animation-delay: {{ 60 + ($loop->index * 35) }}ms;">
                        <td class="en-td-course">{{ $e->course->title ?? 'Deleted Course' }}</td>
                        <td>
                            <div class="en-student-name">{{ $e->user->name ?? 'Deleted User' }}</div>
                            <div class="en-student-email">{{ $e->user->email ?? '-' }}</div>
                        </td>
                        <td class="en-td-progress">
                            <div class="en-progress-row">
                                <div class="en-progress-track">
                                    <div class="en-progress-fill" style="width: {{ min(100, max(0, $e->progress ?? 0)) }}%;"></div>
                                </div>
                                <span class="en-progress-label">{{ $e->progress ?? 0 }}%</span>
                            </div>
                        </td>
                        <td>
                            @if($e->completed)
                        ...
                        <td class="en-td-date">{{ $e->created_at->format('M d, Y') }}</td>
                    </tr>
                @empty
                    <tr>
