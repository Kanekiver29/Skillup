...
<div class="em-page em-fx">
    ...
    <div class="em-fx__header">
        ...
        <div class="em-fx__actions">
            <a href="{{ route('staff.enrollments.index') }}" class="em-fx-btn em-fx-btn--ghost">
                ← Back
            </a>
            <a href="{{ route('staff.enrollments.show', $enrollment->id) }}" class="em-fx-btn">
                View Details
            </a>
        </div>
    </div>
    ...
    <div class="em-fx__card">
        ...
        <form method="POST" action="{{ route('staff.enrollments.update', $enrollment->id) }}" id="fxEnrollForm" novalidate>
            @csrf
            @method('PUT')
                ...
                <div class="em-fx__field">
                    ...
                    <div class="em-fx__control-wrap">
                        <select id="year_level" name="year_level" class="em-fx__select" data-fx-field>
                            ...
                            @for($i=1;$i<=6;$i++)
                                <option value="{{ $i }}" {{ (string)($enrollment->year_level ?? '') === (string)$i ? 'selected' : '' }}>Year {{ $i }}</option>
                            @endfor
                        </select>
                        <span class="em-fx__chevron"></span>
                    </div>
                    @error('year_level')
                        <div class="em-fx__hint em-fx__hint--error"><span>⚠</span>{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="em-fx__field">
                ...
                <div class="em-fx__control-wrap">
                    <select id="status" name="status" class="em-fx__select" data-fx-field>
                        <option value="active" {{ ($enrollment->status ?? 'active') === 'active' ? 'selected' : '' }}>Active</option>
                        ...
                        <option value="complete" {{ ($enrollment->status ?? '') === 'complete' ? 'selected' : '' }}>Completed</option>
                    </select>
                    <span class="em-fx__chevron"></span>
                </div>
                @error('status')
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const leaveUrl = '{{ route('staff.enrollments.index') }}';
