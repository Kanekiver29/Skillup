...
<div class="em-page">
    <a href="{{ route('staff.enrollments.index') }}" class="em-back">
    ...
    <div class="em-header">
        ...
        <div class="em-header__actions">
            <a href="{{ route('staff.enrollments.index') }}" class="em-btn em-btn--outline">
            ...
            <a href="{{ route('staff.enrollments.edit', $enrollment->id) }}" class="em-btn em-btn--primary">
            ...
            <form method="POST" action="{{ route('staff.enrollments.destroy', $enrollment->id) }}" onsubmit="return confirm('Remove this enrollment?')" style="display:inline">
                @csrf
                @method('DELETE')
    ...
    <div class="em-card">
        ...
        <div class="em-profile">
            <div class="em-profile__avatar" style="background: linear-gradient(135deg, hsl({{ $avatarHue }},75%,68%), hsl({{ ($avatarHue + 50) % 360 }},75%,60%));">
                {{ strtoupper(substr($studentName, 0, 1)) }}
            </div>
            <div>
                <div class="em-profile__name">{{ $studentName }}</div>
                <div class="em-profile__email">{{ $studentEmail ?: 'N/A' }}</div>
                <div class="em-profile__id">ID: {{ $studentId }}</div>
            </div>
        </div>
        ...
        <div class="em-course-row">
            <div>
                <div class="em-course-name">{{ $courseName }}</div>
            </div>
            @if($courseCode)
                <span class="em-course-code">{{ $courseCode }}</span>
            @endif
        </div>
        ...
        <div class="em-meta-grid">
            <div class="em-meta-item">
                ...
                <div class="em-meta-item__value">{{ $enrollment->year_level ?? '—' }}</div>
            </div>
            <div class="em-meta-item">
                <div class="em-meta-item__label">
                ...
                <div class="em-meta-item__value">{{ $enrollment->section ?? '—' }}</div>
            </div>
            <div class="em-meta-item">
                <div class="em-meta-item__label">
                ...
                <span class="em-badge em-badge--{{ $statusValue }}">
        ...
        <div class="em-timeline">
            <div class="em-timeline__item">
                ...
                <div class="em-timeline__value">{{ optional($enrollment->enrolled_at)->format('M d, Y H:i') ?? optional($enrollment->created_at)->format('M d, Y H:i') }}</div>
            </div>
            <div class="em-timeline__item">
                <div class="em-timeline__label">
                ...
                <div class="em-timeline__value">{{ optional($enrollment->updated_at)->diffForHumans() ?? '-' }}</div>
            </div>
            <div class="em-progress-wrap">
                ...
                <div class="em-progress-track">
                    <div class="em-progress-fill" id="emProgressFill" data-progress="{{ $progressValue }}"></div>
                </div>
            </div>
        </div>
    </div>
