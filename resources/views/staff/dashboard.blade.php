...
<div class="db-page">
    ...
    <div class="db-banner db-animate" style="animation-delay:0ms">
        <div>
            <div class="db-banner__greeting">{{ $greeting }}</div>
            <div class="db-banner__name">{{ $userName }}</div>
            <div class="db-banner__sub">Here's what's on your plate today. Stay focused.</div>
        </div>
        <div class="db-banner__right">
            <div id="db-clock-badge" class="db-clock-badge {{ $clockedIn ? '' : 'is-out' }}">
                <span id="db-clock-dot" class="db-clock-dot {{ $clockedIn ? 'db-live-dot' : 'is-out' }}"></span>
                <span id="db-clock-text">
                    {{ $clockedIn ? ('Clocked in since '.($clockedInSince ?? now()->format('g:i A'))) : 'Not clocked in' }}
                </span>
            </div>
            <div class="db-banner__date">{{ now()->format('l, F j, Y') }}</div>
        </div>
    </div>
    ...
    <div class="db-stats">
        ...
        <div class="db-stat db-animate" style="animation-delay:{{ 60 + $i*60 }}ms">
            <div class="db-stat__top">
                <div class="db-stat__label">{{ $stat['label'] }}</div>
                <div class="db-stat__icon {{ $stat['iconClass'] }}">
                    {!! $icon($stat['icon']) !!}
                </div>
            </div>
            <div class="db-stat__value">{{ $stat['value'] }}</div>
            <div class="db-stat__sub">{{ $stat['sub'] }}</div>
            @if($stat['progress'] ?? null)
            <div class="db-stat__bar">
                <div class="db-stat__bar-fill" style="{{ $stat['barColor'] ?? 'background:var(--db-accent)' }};width:{{ round($stat['progress']*100) }}%"></div>
            </div>
            @endif
            @if($stat['trend'] ?? null)
            <div class="db-stat__trend db-stat__trend--{{ $stat['trendDir'] ?? 'up' }}">
                {!! $icon($stat['trendDir'] ?? 'up') !!}
                {{ $stat['trend'] }}
            </div>
            @endif
        </div>
    ...
    <div class="db-quick db-animate" style="animation-delay:300ms">
        ...
        <div class="db-quick__grid">
            ...
            {{-- Clock toggle (AJAX) --}}
            <form method="POST" action="{{ route('staff.clock.toggle') }}" id="db-clock-form">
                @csrf
                <button type="submit"
                        id="db-clock-btn"
                        class="db-btn db-btn--primary"
                        aria-pressed="{{ $clockedIn ? 'true' : 'false' }}">
                    {!! $icon('clock') !!}
                    <span id="db-clock-label">{{ $clockedIn ? 'Clock Out' : 'Clock In' }}</span>
            ...
            <a href="{{ route('staff.tasks.create') }}" class="db-btn db-btn--secondary">
                {!! $icon('plus') !!} New Task
            </a>
            <a href="{{ route('staff.leave.request') }}" class="db-btn db-btn--secondary">
                {!! $icon('leave') !!} Request Leave
            </a>
            <a href="{{ route('staff.schedule.index') }}" class="db-btn db-btn--secondary">
                {!! $icon('calendar') !!} Full Schedule
            </a>
            <a href="{{ route('staff.reports.index') ?? '#' }}" class="db-btn db-btn--secondary">
                {!! $icon('report') !!} My Reports
            </a>
            <a href="{{ route('staff.support.index') }}" class="db-btn db-btn--secondary">
    ...
    <div class="db-grid">
        ...
        <div class="db-card db-animate" style="animation-delay:340ms">
                ...
                <table class="db-task-table" aria-label="My tasks">
                    ...
                    <tbody id="db-tasks-body">
                        ...
                        <tr class="db-task-row db-row-fade"
                            ...
                            style="animation-delay:{{ min($i,8)*30 }}ms">
                            <td>
                                <input type="checkbox"
                            ...
                            <td>
                                <span style="color:var(--db-gray-900);font-weight:500">{{ $task['title'] }}</span>
                            </td>
                            <td>
                                <span class="db-badge {{ $badgeClass }}">{{ $statusKey }}</span>
                            </td>
                            <td style="font-size:12.5px;white-space:nowrap">{{ $task['due'] }}</td>
                            <td style="text-align:right">
                                <span class="db-priority db-priority--{{ $priority }}" title="{{ ucfirst($priority) }} priority"></span>
                            </td>
                            <td style="text-align:right">
                                <button type="button"
            ...
            <div class="db-card__footer">
                <a href="{{ route('staff.tasks.index') ?? '#' }}" class="db-link">
                    View all tasks {!! $icon('arrow') !!}
                </a>
            </div>
        </div>
        ...
        <div class="db-sidebar">
            ...
            <div class="db-card db-animate" style="animation-delay:380ms">
                <div class="db-card__header">
                    ...
                    <a href="{{ route('staff.schedule.index') }}" class="db-link">
                        Full view {!! $icon('arrow') !!}
                    </a>
                </div>
                <div class="db-timeline">
                    ...
                    @endphp
                    <div class="db-timeline-item {{ $isNow ? 'is-now' : '' }}">
                        <span class="db-timeline__dot"></span>
                        <div style="flex:1">
                            <div class="db-timeline__time">{{ $item['time'] }}</div>
                        </div>
                        <div style="flex:3">
                            <div class="db-timeline__event">{{ $item['event'] }}</div>
                            @if(!empty($item['sub']))
                            <div class="db-timeline__sub">{{ $item['sub'] }}</div>
                            @endif
                        </div>
                    </div>
                    @endforeach
            ...
            <div class="db-card db-animate" style="animation-delay:420ms">
                <div class="db-card__header">
                    <div style="display:flex;align-items:center;gap:8px">
                        ...
                        <span class="db-notif-badge">{{ count($notifications) }}</span>
                    </div>
                    <button type="button"
                            id="db-mark-all-read"
                ...
                <div class="db-notif-list" id="db-notif-list">
                    ...
                    <div class="db-notif db-notif--{{ $note['type'] ?? 'info' }}">
                        <span class="db-notif__dot"></span>
                        <div>
                            <div class="db-notif__text">{{ $note['text'] }}</div>
                            <div class="db-notif__time">{{ $note['time'] }}</div>
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            <div class="db-card db-animate" style="animation-delay:460ms">
                <div class="db-card__header">
                    ...
                    <a href="{{ route('staff.attendance.index') ?? '#' }}" class="db-link">
                        Details {!! $icon('arrow') !!}
                    </a>
                </div>
                <div class="db-attendance">
                    ...
                    <div class="db-attendance__bar">
                        <div class="db-attendance__fill" style="width:{{ round((32.5/40)*100) }}%"></div>
                    </div>
                    ...
                    <div class="db-attendance__days" style="margin-top:16px">
                        ...
                        <div class="db-att-day db-att-day--{{ $day['state'] }}">{{ $day['label'] }}</div>
                        @endforeach
                    </div>
...
(function () {
    ...
    if (clockForm) {
        clockForm.addEventListener('submit', function (e) {
            ...
            fetch('{{ route("staff.clock.toggle") }}', {
                method: 'POST',
