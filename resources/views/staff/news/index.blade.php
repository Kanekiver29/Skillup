...
<div class="np-wrap" x-data="newsManager()" x-init="init()">
    ...
    <div class="np-stats">
        <div class="np-stat">
            ...
            <div>
                <div class="np-stat__val">{{ $news->count() }}</div>
        ...
        <div class="np-stat">
            ...
            <div>
                <div class="np-stat__val">{{ $news->where('category','alert')->count() }}</div>
        ...
        <div class="np-stat">
            ...
            <div>
                <div class="np-stat__val">{{ $news->where('category','event')->count() }}</div>
        ...
        <div class="np-stat">
            ...
            <div>
                <div class="np-stat__val">{{ number_format($news->sum('view_count')) }}</div>
        ...
        <table class="np-table">
            ...
            <tbody id="news-tbody">
                ...
                <tr class="np-row"
                    ...
                    data-audience="{{ $item->target_audience ?? 'all' }}">
                    <td>
                        <div class="np-title-cell">
                            <span class="np-title-main" title="{{ $item->title }}">{{ $item->title }}</span>
                            <span class="np-title-preview">{{ Str::limit(strip_tags($item->content), 70) }}</span>
                        </div>
                    </td>
                    <td>
                    ...
                    <td>
                        <span class="np-audience">
                            ...
                            {{ str_replace('_', ' ', ucfirst($item->target_audience ?? 'All')) }}
                        </span>
                    </td>
                    <td>
                        @if($item->published_at)
                            <span style="color:var(--n-text); font-size:.82rem;">{{ $item->published_at->format('M d, Y') }}</span>
                            <span style="display:block; font-size:.72rem; color:var(--n-muted2);">{{ $item->published_at->format('h:i A') }}</span>
                        @else
                            <span style="color:var(--n-muted2); font-size:.8rem;">Draft</span>
                        @endif
                    </td>
                    <td>
                        <span class="np-views"><i class="fas fa-eye"></i>{{ number_format($item->view_count ?? 0) }}</span>
                    </td>
                    <td>
                        <div class="np-actions" style="justify-content:flex-end;">
                            ...
                            <button class="np-act-btn del" title="Delete"
                                    @click="openDelete({{ $item->id }}, '{{ addslashes($item->title) }}')">
    ...
    return {
        ...
        init() {
            ...
            @if($errors->any() && old('_method'))
                this.showModal = true;
                this.isEdit = '{{ old("_method") }}' === 'PUT';
                this.form = {
                    id:          {{ old('news_id', 'null') }},
                    title:       '{{ old("title") }}',
                    category:    '{{ old("category", "update") }}',
