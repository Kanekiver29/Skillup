...
<div class="news-page">
    <div class="news-header">
        ...
        <a href="{{ route('staff.news.index') }}" class="db-btn db-btn--ghost">
            <i class="fas fa-arrow-left"></i> Back to List
        </a>
    </div>

    <div class="db-card">
        <form action="{{ route('staff.news.update', $newsItem->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                <div class="form-group">
                    ...
                    <select name="category" class="form-control" required>
                        <option value="update" {{ old('category', $newsItem->category) == 'update' ? 'selected' : '' }}>General Update</option>
                        <option value="academic" {{ old('category', $newsItem->category) == 'academic' ? 'selected' : '' }}>Academic</option>
                        <option value="event" {{ old('category', $newsItem->category) == 'event' ? 'selected' : '' }}>Event</option>
                        <option value="alert" style="color: #FF5D6C;" {{ old('category', $newsItem->category) == 'alert' ? 'selected' : '' }}>Urgent Alert / Push Notification</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select name="audience" class="form-control">
                        <option value="all" {{ old('audience', $newsItem->target_audience) == 'all' ? 'selected' : '' }}>Broadcast to All Students</option>
                        <option value="year_1" {{ old('audience', $newsItem->target_audience) == 'year_1' ? 'selected' : '' }}>1st Year Students</option>
                        <option value="year_2" {{ old('audience', $newsItem->target_audience) == 'year_2' ? 'selected' : '' }}>2nd Year Students</option>
                        <option value="dept_it" {{ old('audience', $newsItem->target_audience) == 'dept_it' ? 'selected' : '' }}>IT Department</option>
                    </select>
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(100px, 1fr)); gap: 12px; margin-bottom: 15px;">
                        @foreach($newsItem->images as $photo)
                            <div style="position: relative; border-radius: var(--db-radius-sm); overflow: hidden; border: 1px solid var(--db-gray-300);">
                                <img src="{{ Storage::url($photo->file_path) }}" alt="Photo" style="width: 100%; height: 100px; object-fit: cover; display: block;">
                                <label style="position: absolute; top: 4px; right: 4px; background: rgba(0,0,0,0.6); padding: 4px; border-radius: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center; margin: 0;">
                                    <input type="checkbox" name="deleted_media[]" value="{{ $photo->id }}" data-type="image" class="delete-checkbox" style="margin: 0;">
            ...
            <div class="form-group">
                ...
                @if($newsItem->videos->isNotEmpty())
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 12px; margin-bottom: 15px;">
                        @foreach($newsItem->videos as $video)
                            <div style="position: relative; border-radius: var(--db-radius-sm); overflow: hidden; border: 1px solid var(--db-gray-300); background: #000; display: flex; align-items: center; justify-content: center; height: 100px;">
                                ...
                                <label style="position: absolute; top: 4px; right: 4px; background: rgba(0,0,0,0.6); padding: 4px; border-radius: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center; margin: 0;">
                                    <input type="checkbox" name="deleted_media[]" value="{{ $video->id }}" data-type="video" class="delete-checkbox" style="margin: 0;">
                                    <span style="color: var(--db-danger); font-size: 10px; margin-left: 4px; font-weight: bold; font-family: var(--db-font);">DEL</span>
                                </label>
                            </div>
                        @endforeach
            ...
            <div class="form-group" style="display: flex; gap: 12px; margin-top: 30px; padding-top: 20px; border-top: 1px solid var(--db-gray-200);">
                ...
                <a href="{{ route('staff.news.index') }}" class="db-btn db-btn--ghost">Cancel</a>
            </div>
        </form>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        const existingPhotos = {{ $newsItem->images->count() }};
        const existingVideos = {{ $newsItem->videos->count() }};
