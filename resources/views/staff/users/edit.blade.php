...
<div class="amc">
    ...
    <div class="amc-header">
        ...
        @endphp
        <div class="amc-avatar-lg" style="background: linear-gradient(135deg, hsl({{ $hue }},75%,68%), hsl({{ ($hue + 50) % 360 }},75%,60%));">{{ strtoupper($initials) }}</div>
        <div>
            <div class="amc-eyebrow"><span class="dot"></span> Access Control / Edit Record</div>
            <h1 class="amc-title">{{ $user->name }}</h1>
            <p class="amc-subtitle">{{ $user->email }}</p>
        </div>
    </div>

    <form action="{{ route('staff.users.update', $user) }}" method="POST" id="amc-edit-form" class="amc-panel">
        ...
        <div class="amc-grid" style="margin-bottom: 26px;">
            <div class="amc-field">
                ...
                <input id="name" name="name" value="{{ old('name', $user->name) }}" class="amc-input @error('name') has-error @enderror">
            ...
            <div class="amc-field">
                ...
                <input id="email" name="email" type="email" value="{{ old('email', $user->email) }}" class="amc-input @error('email') has-error @enderror">
            ...
            <div class="amc-field">
                ...
                <input id="username" name="username" value="{{ old('username', $user->username) }}" class="amc-input @error('username') has-error @enderror">
            ...
            <div class="amc-field">
                ...
                <input id="department" name="department" value="{{ old('department', $user->department) }}" class="amc-input @error('department') has-error @enderror">
            ...
            <div class="amc-field">
                ...
                <select id="assigned_course_id" name="assigned_course_id" class="amc-select @error('assigned_course_id') has-error @enderror">
                    ...
                    @foreach($courses as $c)
                        <option value="{{ $c->id }}" {{ (int) old('assigned_course_id', $user->assigned_course_id) === $c->id ? 'selected' : '' }}>{{ $c->title }}</option>
                    @endforeach
                </select>
    ...
    <form action="{{ route('staff.users.reset-password', $user) }}" method="POST" style="margin-top:14px;">
        @csrf
        <button type="submit" class="amc-btn amc-btn--danger-outline" onclick="return confirm('Reset password for {{ $user->name }}?')">
