...
<div style="max-width: 960px; margin: 0 auto; padding: 1rem 0 2rem;">
    ...
    <div style="background:#fff; border:1px solid #e5edf9; border-radius:1rem; padding:1.5rem; box-shadow:0 12px 30px rgba(30,64,175,0.06);">
        <form action="{{ route('staff.quizzes.update', $quiz) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div style="display:grid; grid-template-columns:repeat(2, minmax(0, 1fr)); gap:1.25rem;">
                ...
                <div style="grid-column:1 / -1;">
                    ...
                    <textarea id="description" name="description" rows="4" style="width:100%; padding:0.8rem 0.9rem; border:1px solid #d9e2f5; border-radius:0.7rem; background:#f8fbff; color:#0f172a; resize:vertical;">{{ old('description', $quiz->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select id="course_id" name="course_id" style="width:100%; padding:0.8rem 0.9rem; border:1px solid #d9e2f5; border-radius:0.7rem; background:#f8fbff; color:#0f172a;">
                        ...
                        @foreach($courses as $course)
                            <option value="{{ $course->id }}" {{ old('course_id', $quiz->module?->course_id) == $course->id ? 'selected' : '' }}>
                                {{ $course->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input id="passing_score" name="passing_score" type="number" min="0" max="100" value="{{ old('passing_score', $quiz->passing_score ?? 60) }}"
                           style="width:100%; padding:0.8rem 0.9rem; border:1px solid #d9e2f5; border-radius:0.7rem; background:#f8fbff; color:#0f172a;" />
                </div>
                ...
                <div>
                    ...
                    <input id="time_limit_minutes" name="time_limit_minutes" type="number" min="0" value="{{ old('time_limit_minutes', $quiz->time_limit_minutes ?? 30) }}"
                           style="width:100%; padding:0.8rem 0.9rem; border:1px solid #d9e2f5; border-radius:0.7rem; background:#f8fbff; color:#0f172a;" />
                </div>
            </div>
