        ...
        <div class="mb-8 rounded-3xl border border-slate-700/70 bg-slate-900/90 p-6 shadow-xl shadow-slate-950/30 backdrop-blur-xl">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex flex-wrap gap-3">
                    <a href="{{ route('staff.lessons.create') }}" class="inline-flex items-center gap-2 rounded-2xl bg-cyan-500 px-5 py-3 text-sm font-semibold text-slate-900 shadow-lg shadow-cyan-500/20 transition hover:bg-cyan-400">
                        <i class="fas fa-plus" aria-hidden="true"></i>
                        New lesson
                    </a>
                    <a href="{{ route('staff.lessons.archived') }}" class="inline-flex items-center gap-2 rounded-2xl border border-slate-700 bg-slate-800 px-5 py-3 text-sm font-semibold text-slate-200 transition hover:border-cyan-400 hover:text-white">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-slate-500">{{ Str::limit($lesson->description, 50) }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-slate-300">{{ $lesson->course?->title ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 text-slate-300">{{ $lesson->module?->title ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 text-slate-300">{{ $lesson->order }}</td>
                                    <td class="px-6 py-4 text-slate-300">{{ $lesson->updated_at->format('M d, Y') }}</td>
                                    <td class="px-6 py-4 text-right">
                                        <a href="{{ route('staff.lessons.edit', $lesson) }}" class="inline-flex items-center gap-2 rounded-full border border-slate-700 bg-slate-950/80 px-4 py-2 text-xs font-semibold text-slate-100 transition hover:border-cyan-400 hover:text-cyan-300">
