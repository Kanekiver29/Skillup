    ...
    <div class="mx-auto max-w-7xl">
        <div class="mb-8 rounded-3xl border border-slate-700/70 bg-slate-900/90 p-6 shadow-xl shadow-slate-950/30 backdrop-blur-xl">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <a href="{{ route('staff.lessons.list') }}" class="inline-flex items-center gap-2 rounded-2xl border border-slate-700 bg-slate-800 px-5 py-3 text-sm font-semibold text-slate-200 transition hover:border-cyan-400 hover:text-white">
                        ...
                        <tbody class="divide-y divide-slate-800 bg-slate-900/80">
                            @foreach($lessons as $lesson)
                                <tr class="hover:bg-slate-950/80 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 text-slate-300">{{ optional($lesson->deleted_at)->format('M d, Y') ?? 'Unknown' }}</td>
