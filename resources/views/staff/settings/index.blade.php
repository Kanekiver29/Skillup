...
<div class="ss-wrap">
    ...
    <div class="ss-card">
        ...
        <div class="ss-tabs" role="tablist" id="ss-tablist">
            ...
            @endphp
            @foreach($ssTabs as $key => $meta)
                <button
                    class="ss-tab{{ $key === 'system' ? ' is-active' : '' }}"
                    role="tab"
                    data-target="{{ $key }}"
                    aria-selected="{{ $key === 'system' ? 'true' : 'false' }}"
                >
                    <span class="ss-tab__icon">{{ $meta['icon'] }}</span>
                    {{ $meta['label'] }}
                </button>
            @endforeach
        </div>
        ...
        <div class="ss-panels">
            ...
            <div class="ss-panel" id="ss-panel-profile" role="tabpanel">
                ...
                <div class="ss-sub-card">
                    ...
                    <form action="{{ route('staff.settings.profile.update') }}" method="POST">
                        @csrf
                        <div class="ss-grid ss-grid--2">
                            <div class="ss-field">
                                <label class="ss-label" for="p_name">Full Name</label>
                                <input id="p_name" class="ss-input" name="name" value="{{ old('name', auth()->user()->name) }}">
                            </div>
                            <div class="ss-field">
                                <label class="ss-label" for="p_email">Email Address</label>
                                <input id="p_email" class="ss-input" name="email" type="email" value="{{ old('email', auth()->user()->email) }}">
                            </div>
                        </div>
                ...
                <div class="ss-sub-card">
                    ...
                    <form action="{{ route('staff.settings.password.change') }}" method="POST">
                        @csrf
                ...
                <div class="ss-sub-card">
                    ...
                    <form action="{{ route('staff.settings.avatar.upload') }}" method="POST" enctype="multipart/form-data">
                        @csrf
            ...
            <div class="ss-panel" id="ss-panel-account" role="tabpanel">
                <div class="ss-sub-card">
                    ...
                    <form action="{{ route('staff.settings.account.update') }}" method="POST">
                        @csrf
                        <div class="ss-grid ss-grid--2">
                            <label class="ss-check">
                                <input type="checkbox" name="receive_notifications" value="1"
                                    {{ session('staff_account_settings.receive_notifications') ? 'checked' : '' }}>
            ...
            <div class="ss-panel" id="ss-panel-organization" role="tabpanel">
                <div class="ss-sub-card">
                    ...
                    <form action="{{ route('staff.settings.organization.update') }}" method="POST">
                        @csrf
                        <div class="ss-grid ss-grid--2">
                            <div class="ss-field">
                                <label class="ss-label">School / Training Center Name</label>
                                <input class="ss-input" name="org_name"
                                    value="{{ old('org_name', session('staff_org_info.org_name')) }}">
                            </div>
                            <div class="ss-field">
                                <label class="ss-label">Contact</label>
                                <input class="ss-input" name="org_contact"
                                    value="{{ old('org_contact', session('staff_org_info.org_contact')) }}">
                            </div>
                            <div class="ss-field ss-field--span2">
                                <label class="ss-label">Address</label>
                                <textarea class="ss-input" name="org_address">{{ old('org_address', session('staff_org_info.org_address')) }}</textarea>
                            </div>
                        </div>
                        <div class="ss-form-footer">
            ...
            <div class="ss-panel" id="ss-panel-courses" role="tabpanel">
                <div class="ss-sub-card">
                    ...
                    <form action="{{ route('staff.settings.courses.update') }}" method="POST">
                        @csrf
                        <div class="ss-grid ss-grid--2">
                            <div class="ss-field">
                                <label class="ss-label">Default Category</label>
                                <input class="ss-input" name="default_category"
                                    value="{{ old('default_category', session('staff_course_settings.default_category')) }}">
                            </div>
                            <label class="ss-check" style="align-self:flex-end">
                                <input type="checkbox" name="certificate_required" value="1"
                                    {{ session('staff_course_settings.certificate_required') ? 'checked' : '' }}>
            ...
            <div class="ss-panel" id="ss-panel-certificates" role="tabpanel">
                <div class="ss-sub-card">
                    ...
                    <form action="{{ route('staff.settings.certificates.update') }}" method="POST">
                        @csrf
                        <div class="ss-grid ss-grid--2">
                            <div class="ss-field">
                                <label class="ss-label">Certificate Prefix / Number Format</label>
                                <input class="ss-input" name="certificate_prefix"
                                    value="{{ old('certificate_prefix', session('staff_certificate_settings.certificate_prefix')) }}"
                                    placeholder="e.g. CERT-2025-">
                            </div>
                            <div class="ss-field">
                                <label class="ss-label">Signature / Approver Name</label>
                                <input class="ss-input" name="signature_name"
                                    value="{{ old('signature_name', session('staff_certificate_settings.signature_name')) }}"
            ...
            <div class="ss-panel" id="ss-panel-notifications" role="tabpanel">
                <div class="ss-sub-card">
                    ...
                    <form action="{{ route('staff.settings.notifications.update') }}" method="POST">
                        @csrf
                        <div class="ss-grid ss-grid--2" style="gap:.75rem">
                            <label class="ss-check">
                                <input type="checkbox" name="email_notifications" value="1"
                                    {{ session('staff_notification_settings.email_notifications') ? 'checked' : '' }}>
                                📧 Email Notifications
                            </label>
                            <label class="ss-check">
                                <input type="checkbox" name="sms_notifications" value="1"
                                    {{ session('staff_notification_settings.sms_notifications') ? 'checked' : '' }}>
                                📱 SMS Notifications
                            </label>
                            <label class="ss-check">
                                <input type="checkbox" name="announcements_enabled" value="1"
                                    {{ session('staff_notification_settings.announcements_enabled') ? 'checked' : '' }}>
                                📢 Announcements
                            </label>
                        </div>
                        <div class="ss-form-footer">
            ...
            <div class="ss-panel" id="ss-panel-security" role="tabpanel">
                <div class="ss-sub-card">
                    ...
                    <form action="{{ route('staff.settings.security.update') }}" method="POST">
                        @csrf
                        <div class="ss-grid ss-grid--2">
                            <div class="ss-field">
                                ...
                                <input class="ss-input" name="password_policy_min_length" type="number" min="6" max="128"
                                    value="{{ session('staff_security_settings.password_policy_min_length', 8) }}">
                            </div>
                            <div class="ss-field">
                                ...
                                <input class="ss-input" name="lockout_attempts" type="number" min="1" max="20"
                                    value="{{ session('staff_security_settings.lockout_attempts', 5) }}">
                            </div>
                        </div>
                        <div class="ss-form-footer">
            ...
            <div class="ss-panel" id="ss-panel-activity" role="tabpanel">
                ...
                <pre class="ss-pre" style="margin-top:1rem">{{ json_encode([
            ...
            <div class="ss-panel is-active" id="ss-panel-system" role="tabpanel">
                ...
                <div class="ss-stats">
                    <div class="ss-stat">
                        ...
                        <div class="ss-stat__value">{{ PHP_VERSION }}</div>
                    </div>
                    <div class="ss-stat">
                        <div class="ss-stat__label">Laravel</div>
                        <div class="ss-stat__value">{{ app()->version() }}</div>
                    </div>
                    <div class="ss-stat">
                        <div class="ss-stat__label">Server OS</div>
                        <div class="ss-stat__value">{{ PHP_OS }}</div>
                    </div>
                    <div class="ss-stat">
                        <div class="ss-stat__label">Memory Limit</div>
                        <div class="ss-stat__value">{{ ini_get('memory_limit') }}</div>
                    </div>
                </div>
                ...
                <div class="ss-sub-card">
                    <div class="ss-info-grid">
                        <div class="ss-info-item">
                            ...
                            <div class="ss-info-item__value">{{ $settings['app_name'] ?? config('app.name') }}</div>
                        </div>
                        <div class="ss-info-item">
                            <div class="ss-info-item__label">Environment</div>
                            <div class="ss-info-item__value">
                                @php $env = $settings['app_env'] ?? config('app.env'); @endphp
                                <span class="ss-badge ss-badge--env-{{ $env }}">{{ ucfirst($env) }}</span>
                            </div>
                        </div>
                        <div class="ss-info-item">
                            <div class="ss-info-item__label">App URL</div>
                            <div class="ss-info-item__value" style="font-size:.82rem;word-break:break-all">{{ $settings['app_url'] ?? config('app.url') }}</div>
                        </div>
                        <div class="ss-info-item">
                            <div class="ss-info-item__label">Debug Mode</div>
                            <div class="ss-info-item__value">
                                @if($settings['app_debug'] ?? config('app.debug'))
                ...
                <div class="ss-sub-card">
                    <div class="ss-info-grid">
                        <div class="ss-info-item">
                            ...
                            <div class="ss-info-item__value">{{ $settings['cache_driver'] ?? 'file' }}</div>
                        </div>
                        <div class="ss-info-item">
                            <div class="ss-info-item__label">Session Driver</div>
                            <div class="ss-info-item__value">{{ $settings['session_driver'] ?? 'file' }}</div>
                        </div>
                        <div class="ss-info-item">
                            <div class="ss-info-item__label">Queue Connection</div>
                            <div class="ss-info-item__value">{{ $settings['queue_connection'] ?? 'sync' }}</div>
                        </div>
                        <div class="ss-info-item">
                            <div class="ss-info-item__label">Filesystem</div>
                            <div class="ss-info-item__value">{{ $settings['filesystems'] ?? 'local' }}</div>
                        </div>
                    </div>
                </div>
