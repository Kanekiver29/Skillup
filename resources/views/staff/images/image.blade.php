...
<div class="ir-page">
    ...
    <div class="ir-card ir-form-card">
        ...
        <form action="{{ route('staff.images.store') }}" method="POST" enctype="multipart/form-data" id="image_form" class="ir-form-grid">
            @csrf
        ...
        <table class="ir-table">
            ...
            <tbody>
                @forelse($modules as $module)
                    <tr class="ir-row" style="animation-delay: {{ 150 + ($loop->index * 40) }}ms;">
                        <td class="ir-td-module">{{ $module->course->title ?? 'Course' }} · {{ $module->title }}</td>
                        <td>
                            @if($module->image_url)
                                <a href="{{ $module->image_url }}" target="_blank" rel="noopener" class="ir-thumb-link">
                                    <img src="{{ $module->image_url }}" alt="{{ $module->title }} thumbnail" class="ir-thumb-img">
                        ...
                        <td class="ir-td-actions">
                            <a href="{{ route('staff.images.edit', $module) }}" class="ir-link-edit">Edit</a>
                            <form action="{{ route('staff.images.destroy', $module) }}" method="POST" onsubmit="return confirm('Remove this image resource?');" class="ir-inline-form">
                                @csrf
                                @method('DELETE')
