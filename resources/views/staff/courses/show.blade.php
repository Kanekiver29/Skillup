    ...
    <div class="cr-card" style="padding:24px;display:grid;gap:16px;">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
            <div>
                ...
                <h1 class="cr-header__title" style="margin:4px 0 0;">{{ $course->title ?? 'Course Details' }}</h1>
                <p class="cr-header__subtitle" style="margin-top:6px;">Review course information and manage publishing status.</p>
            </div>
            <div style="display:flex;gap:10px;flex-wrap:wrap;">
                <a href="{{ route('staff.courses.edit', $course->id) }}" class="cr-btn cr-btn--outline">Edit</a>
                <a href="{{ route('staff.courses.index') }}" class="cr-btn cr-btn--primary">Back to Courses</a>
            </div>
        </div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px;">
            <div class="cr-card" style="padding:16px;background:#f8fafc;">
                ...
                <div style="font-weight:700;font-size:16px;margin-top:6px;">{{ $course->title ?? '—' }}</div>
            </div>
            <div class="cr-card" style="padding:16px;background:#f8fafc;">
                <div style="font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#64748b;">Category</div>
                <div style="font-weight:700;font-size:16px;margin-top:6px;">{{ $course->category ?? '—' }}</div>
            </div>
            <div class="cr-card" style="padding:16px;background:#f8fafc;">
                <div style="font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#64748b;">Level</div>
                <div style="font-weight:700;font-size:16px;margin-top:6px;">{{ $course->level ?? '—' }}</div>
            </div>
            <div class="cr-card" style="padding:16px;background:#f8fafc;">
                <div style="font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#64748b;">Status</div>
                <div style="font-weight:700;font-size:16px;margin-top:6px;">{{ $course->is_published ? 'Published' : 'Draft' }}</div>
            </div>
        </div>
        ...
        <div class="cr-card" style="padding:16px;background:#fff;">
            ...
            <div style="margin-top:8px;color:#334155;line-height:1.6;">{{ $course->description ?? $course->short_description ?? 'No description provided.' }}</div>
        </div>

        <div class="cr-card" style="padding:16px;background:#fff;">
            <div style="font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#64748b;">Instructor</div>
            <div style="margin-top:8px;font-weight:600;">{{ $course->instructor_name ?? '—' }}</div>
            <div style="margin-top:4px;color:#64748b;">{{ $course->instructor_title ?? '' }}</div>
        </div>
    </div>
</div>
@endsection
