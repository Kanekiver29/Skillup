...
<div class="cr-page">
    ...
    <div class="cr-header cr-animate">
        ...
        <div class="cr-header__actions">
            ...
            @if($exportUrl)
                <a href="{{ $exportUrl }}" class="cr-btn cr-btn--outline">
    ...
    <div class="cr-stats cr-animate" style="animation-delay:60ms">
        <div class="cr-stat cr-stat--indigo cr-hud">
            ...
            <div class="cr-stat__value" data-count-to="{{ (int) $courseCount }}">0</div>
        ...
        <div class="cr-stat cr-stat--green cr-hud">
            ...
            <div class="cr-stat__value" data-count-to="{{ (int) $activeCourses }}">0</div>
        ...
        <div class="cr-stat cr-stat--amber cr-hud">
            ...
            <div class="cr-stat__value" data-count-to="{{ (int) $instructorCount }}">0</div>
            <div class="cr-stat__label">Instructors</div>
        </div>
    </div>

    {{-- ── Toolbar ── --}}
    <form method="GET" action="{{ route('staff.courses.index') }}" class="cr-toolbar cr-animate cr-hud" style="animation-delay:90ms" id="crFilterForm">
        <span class="cr-corner cr-corner--tl"></span>
        <span class="cr-corner cr-corner--tr"></span>
        <span class="cr-corner cr-corner--bl"></span>
        <span class="cr-corner cr-corner--br"></span>
        ...
        @if(request()->hasAny(['search','status','sort']))
            <a href="{{ route('staff.courses.index') }}" class="cr-btn cr-btn--outline cr-btn--sm">
                Clear
            </a>
        @endif
        ...
        <div class="cr-card cr-animate cr-hud" style="animation-delay:110ms">
            ...
            <div class="cr-table-wrap">
                <table class="cr-table" aria-label="Course records">
                    ...
                    <tbody>
                        ...
                        @endphp
                        <tr class="cr-row-fade" style="animation-delay:{{ min($loop->index, 14) * 35 }}ms">
                            <td>
                                <input type="checkbox" name="selected[]" value="{{ $course->id }}" aria-label="Select {{ $title }}">
                            </td>
                            <td>
                                <div class="cr-course-cell">
                                    <div class="cr-course-icon" style="background:{{ $iconColor }}">
                                        {{ $initials }}
                                    </div>
                                    <div>
                                        <div class="cr-course-title">{{ $title }}</div>
                                        @if($code)
                                        <div class="cr-course-code">{{ $code }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td>
                                ...
                                <div class="cr-instructor">
                                    <div class="cr-instructor__avatar">{{ $instrInitial }}</div>
                                    {{ $instructor }}
                                </div>
                                @else
                            ...
                            <td style="color:var(--cr-text);font-weight:500">{{ $units }}</td>
                            <td style="white-space:nowrap;color:var(--cr-text-dim)">{{ $createdFormatted }}</td>
                            <td>
                                <span class="cr-badge cr-badge--{{ $status }}">
                            ...
                            <td>
                                <div class="cr-actions">
                                    ...
                                    <a href="{{ route('staff.courses.edit', $course->id) }}"
                                       class="cr-edit-link" aria-label="Edit {{ $title }}">
                                    ...
                                    <form method="POST" action="{{ route('staff.courses.destroy', $course->id) }}"
                                          onsubmit="return confirmDelete(event, '{{ addslashes($title) }}')">
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="cr-pagination">
                ...
                <div class="cr-pagination__pages">
                    ...
                    @else
                        <a href="{{ $courses->previousPageUrl() }}" class="cr-page-btn" aria-label="Previous page">
                            <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M8.5 10.5l-3-3.5 3-3.5" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/></svg>
                        </a>
                    @endif

                    {{-- Page numbers (works with LengthAwarePaginator) --}}
                    @if(method_exists($courses,'getUrlRange'))
                        @foreach($courses->getUrlRange(max(1,$courses->currentPage()-2), min($courses->lastPage(),$courses->currentPage()+2)) as $page => $url)
                            <a href="{{ $url }}" class="cr-page-btn {{ $page == $courses->currentPage() ? 'is-active' : '' }}" aria-label="Page {{ $page }}" {{ $page == $courses->currentPage() ? 'aria-current=page' : '' }}>
                                {{ $page }}
                            </a>
                        @endforeach
                    @endif
                    ...
                    @if($courses->hasMorePages())
                        <a href="{{ $courses->nextPageUrl() }}" class="cr-page-btn" aria-label="Next page">
    ...
    @else
        ...
        <div class="cr-empty cr-animate cr-hud" style="animation-delay:110ms">
            ...
            @if(request()->hasAny(['search','status']))
                <a href="{{ route('staff.courses.index') }}" class="cr-btn cr-btn--outline">Clear filters</a>
            @else
    ...
    <div id="courseModal"
        ...
        <div class="cr-modal" role="document">
            ...
            <form id="courseForm" method="POST" action="{{ route('staff.courses.store') }}">
                @csrf
    ...
    {{-- ── Toast stack (JS-driven feedback) ── --}}
