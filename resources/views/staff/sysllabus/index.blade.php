...
@section('title', 'Syllabi Management')
...
<div class="syl-wrap" style="position:relative; min-height:100vh; padding: 1.5rem 1.5rem 3rem;">
    ...
    <div class="syl-page-header">
        ...
        <a href="{{ route('staff.syllabi.create') }}" class="syl-new-btn">
    ...
    <div class="syl-stats">
        ...
        <div class="syl-stat-card">
            ...
            <div>
                <div class="syl-stat-val" style="color:#34D399;">{{ $published }}</div>
        ...
        <div class="syl-stat-card">
            ...
            <div>
                <div class="syl-stat-val" style="color:#94A3B8;">{{ $drafts }}</div>
        ...
        <div class="syl-panel-content">
            ...
            <div class="syl-toolbar">
                <div class="syl-toolbar-left">
                    ...
                    <span class="syl-toolbar-title">syllabi.index — {{ $total }} {{ Str::plural('record', $total) }}</span>
                </div>
                    ...
                    <tbody>
                        @forelse($syllabi as $syllabus)
                            <tr class="syl-row" style="--i:{{ $loop->index }};">
                                ...
                                <td style="color:#CBD5E1; font-weight:500;">{{ $syllabus->title }}</td>
                                <td>
                                ...
                                <td>
                                    @if($syllabus->effective_start)
                                        <span class="syl-date">
                                            {{ $syllabus->effective_start->format('M d, Y') }}
                                            @if($syllabus->effective_end)
                                                <span class="syl-date-arrow">→</span>{{ $syllabus->effective_end->format('M d, Y') }}
                                            @endif
                                        </span>
                                    @else
                                ...
                                <td>
                                    <div style="display:flex; align-items:center; justify-content:center; gap:4px;">
                                        ...
                                        <a href="{{ route('staff.syllabi.edit', $syllabus) }}"
                                        ...
                                        <form action="{{ route('staff.syllabi.publish', $syllabus) }}"
                                              method="POST" style="display:inline;">
                                            @csrf
                                            <button type="submit"
                                                    class="syl-act {{ $syllabus->is_published ? 'syl-act-unpub' : 'syl-act-pub' }}"
                                                    data-tip="{{ $syllabus->is_published ? 'Unpublish' : 'Publish' }}">
                                                @if($syllabus->is_published)
                                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <form action="{{ route('staff.syllabi.destroy', $syllabus) }}"
                                              method="POST" style="display:inline;"
