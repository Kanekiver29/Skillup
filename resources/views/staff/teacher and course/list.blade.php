...
<div class="tsa-wrap">
    ...
    <div class="tsa-content">
        ...
        <div class="tsa-header">
            ...
            <a href="{{ route('staff.teacher-subjects.create') }}" class="tsa-new-btn">
        ...
        <div class="tsa-stats">
            <div class="tsa-stat-card">
                ...
                <div class="tsa-stat-info">
                    <span class="tsa-stat-value">{{ $totalSubjects ?? 0 }}</span>
            ...
            <div class="tsa-stat-card">
                ...
                <div class="tsa-stat-info">
                    <span class="tsa-stat-value">{{ $assignedCount ?? 0 }}</span>
            ...
            <div class="tsa-stat-card">
                ...
                <div class="tsa-stat-info">
                    <span class="tsa-stat-value">{{ $unassignedCount ?? 0 }}</span>
        ...
        <!-- Filter Bar -->
        <form action="{{ route('staff.teacher-subjects.index') }}" method="GET" class="tsa-filters">
            <div class="tsa-form-group" style="flex: 2;">
                <input type="text" name="search" class="tsa-input" placeholder="Search subject code, title or teacher..." value="{{ request('search') }}">
            ...
            @if(request()->hasAny(['search', 'course_id', 'status']))
                <a href="{{ route('staff.teacher-subjects.index') }}" class="tsa-clear-link">Clear</a>
            @endif
        </form>
                    ...
                    <tbody>
                        @foreach($subjects as $index => $subject)
                            <tr>
                                ...
                                <td><span class="tsa-code">{{ $subject->subject_code }}</span></td>
                                <td style="font-weight:500; color:var(--text);">{{ $subject->title }}</td>
                                <td>
                                    @if($subject->course)
                                        <span class="tsa-badge tsa-badge-course">{{ $subject->course->title }}</span>
                                    @else
                                        -
                                    @endif
                                </td>
                                ...
                                <td>
                                    <div class="tsa-actions" style="justify-content:flex-end;">
                                        @if($subject->teacher_id)
                                            <a href="{{ route('staff.teacher-subjects.edit', $subject->id) }}" class="tsa-action-btn edit" title="Edit Assignment">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('staff.teacher-subjects.destroy', $subject->id) }}" method="POST" class="unassign-form" style="display:inline-block;">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        @else
                                            <a href="{{ route('staff.teacher-subjects.create', ['subject_id' => $subject->id]) }}" class="tsa-action-link">
                                                Assign
                                            </a>
                                        @endif
                                    </div>
