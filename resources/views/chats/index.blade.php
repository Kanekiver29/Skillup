...
@section('title', 'Messages - SkillUp')
  ...
  <aside class="sidebar">
    ...
    <div class="thread-list" id="threadList">
      @forelse($conversations as $conversation)
        ...
        @endphp
        <a class="thread-item {{ $isActive ? 'active' : '' }}" href="{{ route('chats.index', ['conversation' => $conversation->id]) }}">
          <div class="av-wrap">
            <div class="av green" style="width:52px;height:52px;font-size:19px">{{ strtoupper(substr($participant?->name ?? 'S', 0, 1)) }}</div>
          ...
          <div class="t-info">
            <div class="t-top">
              <span class="t-name">{{ $participant?->name ?? 'Support team' }}</span>
              <span class="t-time {{ $latest && $latest->created_at->isToday() ? '' : 'u' }}">{{ $latest?->created_at?->format('H:i') ?? 'now' }}</span>
            </div>
            <div class="t-prev">
              <span class="prev-txt {{ $latest ? '' : 'b' }}">{{ $preview }}</span>
              @if($conversation->user_unread_count > 0)
                <span class="ubadge">{{ $conversation->user_unread_count }}</span>
              @endif
            </div>
          </div>
        </a>
  ...
  <main class="chat-main" id="chatMain">
    @if($selectedConversation && $otherParticipant)
      <div class="chat-header" id="chatHeader">
        <div class="av-wrap">
          <div class="av green" id="headerAv" style="width:42px;height:42px;font-size:16px">{{ strtoupper(substr($otherParticipant->name, 0, 1)) }}</div>
          <span class="odot" id="headerDot" style="width:11px;height:11px;display:block"></span>
        </div>
        <div class="ch-info">
          <div class="ch-name" id="headerName">{{ $otherParticipant->name }}</div>
      ...
      <div class="msgs" id="msgsArea">
        @forelse($messages as $message)
          <div class="br {{ $message->sender_id === auth()->id() ? 'mine' : 'theirs' }}">
            @if($message->sender_id !== auth()->id())
              <div class="av-sm av green">{{ strtoupper(substr($message->sender->name ?? 'S', 0, 1)) }}</div>
            @endif
            <div>
              <div class="bub {{ $message->sender_id === auth()->id() ? 'mine' : 'theirs' }}">{{ $message->body }}</div>
            </div>
          </div>
        @empty
      ...
      <form class="composer" id="composerForm" action="{{ route('chats.store') }}" method="POST">
        @csrf
