@extends('layout.staff.system')

@section('title', 'Staff Dashboard - SkillUp')
    ...
    <header data-admin-animate class="admin-page-hero p-6 md:p-8 text-white admin-page-hero">
        <div class="relative z-[2] flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-slate-300 text-sm mt-1">Welcome back, <span class="font-semibold text-white">{{ auth()->user()->name }}</span></p>
                @php $staffType = \App\Models\User::STAFF_TYPES[auth()->user()->staff_type] ?? null; @endphp
                @if($staffType)
                    <span class="inline-flex items-center mt-2 px-3 py-1 rounded-full bg-white/15 text-xs font-semibold text-sky-100 border border-white/20">
                        <i class="{{ $staffType['icon'] }} mr-1.5"></i>{{ $staffType['label'] }}
                    </span>
                @endif
            </div>
            <div class="text-left sm:text-right rounded-xl bg-white/6 border border-white/8 px-4 py-3 backdrop-blur-sm last-updated-box">
                ...
                <p class="text-sm font-semibold text-cyan-300 tabular-nums" id="last-updated">{{ now()->format('M d, Y - h:i A') }}</p>
            </div>
        </div>
    </header>
    ...
    <div class="space-y-6">
        <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div data-admin-animate class="admin-delay-2 staff-action-card admin-card p-6 text-center rounded-xl bg-[#071726] border border-[#0e2a3b] text-white"
                 onclick="window.location.href='{{ route('admin.modules.index') }}'">
        ...
        <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div data-admin-animate class="admin-stat-card p-5 bg-[#071726] text-white rounded-xl border border-[#0e2a3b]">
                ...
                <p class="text-3xl font-bold text-cyan-300" data-admin-count="{{ $activeCourses }}">{{ $activeCourses }}</p>
            ...
            <div data-admin-animate class="admin-delay-1 admin-stat-card p-5 bg-[#071726] text-white rounded-xl border border-[#0e2a3b]">
                ...
                <p class="text-3xl font-bold text-cyan-300" data-admin-count="{{ $enrollmentCount }}">{{ $enrollmentCount }}</p>
                <p class="text-xs text-slate-400 mt-1">Active enrollments</p>
                @if(isset($newEnrollmentsToday))
                    <p class="text-xs text-emerald-400 mt-1 font-semibold">+{{ $newEnrollmentsToday }} today</p>
                @endif
            </div>
            ...
            <div data-admin-animate class="admin-delay-2 admin-stat-card p-5 bg-[#071726] text-white rounded-xl border border-[#0e2a3b]">
                ...
                <p class="text-3xl font-bold text-cyan-300" data-admin-count="{{ $badges }}">{{ $badges }}</p>
            ...
            <div data-admin-animate class="admin-delay-3 admin-stat-card p-5 bg-[#071726] text-white rounded-xl border border-[#0e2a3b]">
                ...
                <p class="text-3xl font-bold text-cyan-300">{{ $progress }}%</p>
                <div class="w-full bg-[#0b1620] rounded-full h-2 mt-3 overflow-hidden">
                    <div class="staff-progress-bar h-2 rounded-full progress-fill-gradient" style="width: {{ $progress }}%;"></div>
                </div>
            </div>
        </section>
        ...
        <section class="mb-8">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-[#071726] text-white rounded-lg shadow border border-[#0e2a3b] p-6">
                    ...
                    <div class="text-3xl font-bold text-cyan-300">{{ $totalModules }}</div>
                ...
                <div class="bg-[#071726] text-white rounded-lg shadow border border-[#0e2a3b] p-6">
                    ...
                    <div class="text-3xl font-bold text-cyan-300">{{ $totalLessons }}</div>
                ...
                <div class="bg-[#071726] text-white rounded-lg shadow border border-[#0e2a3b] p-6">
                    ...
                    <div class="text-3xl font-bold text-cyan-300">{{ $publishedCourses }}</div>
                        ...
                        <div class="p-4 rounded-lg bg-[#081423] border border-[#112233]">
                            <div class="flex items-center justify-between gap-4">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-400 mt-1">{{ $course->enrollments_count }} enrollments</p>
                                </div>
                                <div class="w-52">
                                    <div class="flex items-center justify-between mb-1">
                                        <small class="text-xs text-slate-400">Progress</small>
                                        <small class="text-xs text-slate-300">{{ intval($course->progress ?? 0) }}%</small>
                                    </div>
                                    <div class="w-full bg-[#06101a] rounded-full h-2 overflow-hidden">
                                        <div class="h-2 rounded-full progress-fill-gradient" style="width: {{ intval($course->progress ?? 0) }}%;"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="divide-y divide-custom-border">
                    @foreach($recentEnrollments as $enrollment)
                        <div class="py-4">
                            <div class="flex items-center justify-between gap-4">
                                <div>
                                    <p class="font-medium text-white">{{ $enrollment->user->name ?? 'Unknown' }}</p>
                                    <p class="text-xs text-slate-400">{{ $enrollment->course->title ?? 'Unknown course' }}</p>
                                </div>
                                <span class="text-xs text-slate-400">{{ $enrollment->created_at->format('M d') }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
...
<script>
    ...
    function loadModules() {
        fetch('{{ route('admin.content.modules') }}', {
            headers: {
        ...
        if (files.length > 0) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('{{ route('admin.content.images') }}', {
                        method: 'POST',
                        headers: {
                            'Accept': 'application/json',
    ...
    function createAssessment() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                submitForm('{{ route('admin.content.assessment') }}', {
    ...
    function createVideo() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                submitForm('{{ route('admin.content.video') }}', {
                    module_id: result.value.moduleId,
                    title: result.value.title,
    ...
    function createPresentation() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                submitFormData('{{ route('admin.content.presentation') }}', formData);
            }
        });
    }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                if (result.value.mediaFile) {
                    ...
                    submitFormData('{{ route('admin.content.quiz') }}', formData);
                } else {
                    submitForm('{{ route('admin.content.quiz') }}', {
                        module_id: result.value.moduleId,
                        title: result.value.title,
    ...
    function createPPT() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                submitForm('{{ route('admin.content.powerpoint') }}', {
                    module_id: result.value.moduleId,
                    title: result.value.title,
