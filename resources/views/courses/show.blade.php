            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                ...
                <div class="md:col-span-2 su-reveal">
                    <div class="mb-4 flex flex-wrap gap-3">
                        <span class="su-badge">{{ $course->category }}</span>
                        <span class="su-badge su-badge-alt">{{ $course->level }}</span>
                    </div>
                    <span class="su-eyebrow">Course Transmission</span>
                    <h1 class="su-course-title su-heading su-gradient-text">{{ $course->title }}</h1>
                    <p class="su-course-desc">{{ $course->short_description }}</p>

                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div class="su-meta-row">
                            <div><i class="fas fa-star" style="color:#ffcf4d;"></i><span class="font-semibold">{{ number_format($course->rating, 1) }}/5.0</span></div>
                            <div><i class="fas fa-users"></i><span>{{ number_format($course->students_count) }} students</span></div>
                            <div><i class="fas fa-clock"></i><span>{{ $course->duration_hours }} hours</span></div>
                        </div>
                    ...
                    <div class="su-telemetry">
                        ...
                        <span>{{ $course->modules->count() }} modules · {{ $course->lessons->count() }} lessons queued</span>
                    </div>
                </div>
                ...
                <div class="su-instructor-card su-reveal su-reveal-delay-1">
                    ...
                    <h3 class="su-heading">{{ $course->instructor_name }}</h3>
        ...
        <div class="max-w-6xl mx-auto">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-5">
                <div class="su-stat-card su-reveal">
                    <div class="su-stat-num"><span class="su-count-up" data-target="{{ $course->modules->count() }}">0</span></div>
                ...
                <div class="su-stat-card su-reveal su-reveal-delay-1">
                    <div class="su-stat-num">{{ number_format($course->rating, 1) }} <i class="fas fa-star" style="color:#ffcf4d; font-size:1rem;"></i></div>
                    <p class="su-stat-label">Rating</p>
                    <p class="su-stat-sub">{{ number_format($course->students_count) }} reviews</p>
                </div>

                <div class="su-stat-card su-reveal su-reveal-delay-2">
                    <div class="su-stat-num capitalize" style="font-size:1.15rem;">{{ $course->level }}</div>
                ...
                <div class="su-stat-card su-reveal su-reveal-delay-3">
                    <div class="su-stat-num"><span class="su-count-up" data-target="{{ $course->duration_hours }}">0</span>h</div>
                ...
                <div class="su-stat-card su-reveal su-reveal-delay-4">
                    <div class="su-stat-num"><span class="su-count-up" data-target="{{ ceil($course->rating * 10) }}">0</span>% <i class="fas fa-thumbs-up" style="color: var(--c-secondary); font-size:1rem;"></i></div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
                ...
                <section class="mb-14 su-reveal">
                    ...
                    <p class="su-body-copy whitespace-pre-line">{{ $course->description }}</p>
                </section>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3 su-reveal">
                        ...
                        <div class="flex flex-wrap gap-3">
                            ...
                            @if(auth()->check() && auth()->user()->hasStaffAccess())
                                <a href="{{ route('admin.modules.create', ['course_id' => $course->id]) }}" class="su-btn su-btn-green su-magnetic"><i class="fas fa-plus"></i>Add Topic</a>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        @forelse($course->modules as $module)
                            <div class="su-module-card su-reveal">
                                <span class="su-module-index">MOD // {{ str_pad($loop->iteration, 2, '0', STR_PAD_LEFT) }}</span>
                                ...
                                <div class="su-module-thumb">
                                    ...
                                    @endphp
                                    @if($topicImage)
                                        <img src="{{ $topicImage }}" alt="{{ $module->title }} topic image">
                                    @else
                                ...
                                <div class="su-module-body">
                                    <div class="flex items-center justify-between mb-2">
                                        <h3>{{ $module->title }}</h3>
                                        <span class="su-lesson-count">{{ $module->lessons->count() }} lessons</span>
                                    </div>
                                    <p class="su-module-desc">{{ \Illuminate\Support\Str::limit($module->description ?? 'Learn the key concepts and examples for this topic.', 120) }}</p>
                                    <div class="flex flex-wrap gap-3 items-center">
                                        <a href="{{ route('modules.show', [$course->slug, $module->slug]) }}" class="su-btn su-btn-solid su-magnetic"><i class="fas fa-eye"></i>Open Topic</a>
                                        @if($module->quizzes->count() > 0)
                                            @php $quiz = $module->quizzes->first(); @endphp
                                            <a href="{{ route('quizzes.start', [$course->slug, $module->slug, $quiz->slug]) }}" class="su-btn su-btn-green su-magnetic"><i class="fas fa-clipboard-list"></i>Take Quiz</a>
                                        @endif
                                        <span class="su-resource-pill">{{ $module->lessons->count() }} resources</span>
                                    </div>
                                </div>
                            </div>
                        @empty
                            <div class="su-empty-state" style="grid-column: 1 / -1;">
                                ...
                                @if(auth()->check() && auth()->user()->hasStaffAccess())
                                    <a href="{{ route('admin.modules.create', ['course_id' => $course->id]) }}" class="su-btn su-btn-green mt-4 su-magnetic"><i class="fas fa-plus"></i>Add first topic</a>
                                @endif
                            </div>
                        @endforelse
                    </div>
                ...
                <div class="su-side-card sticky top-24 su-reveal">
                    ...
                    @if(!empty($course->image_url))
                        <div class="su-side-thumb">
                            <img src="{{ asset($course->image_url) }}" alt="{{ $course->title }} preview">
                        </div>
                    @else
                        ...
                        @if($enrollment)
                            <div class="su-status-panel su-status-info">
                                ...
                                <div class="su-progress-track">
                                    <div class="su-progress-fill su-progress-fill-anim" data-progress="{{ $enrollment->progress }}"></div>
                                </div>
                            </div>
                            ...
                            @if($enrollment->completed)
                                <div class="su-status-panel su-status-gold">
                                    ...
                                    <a href="{{ route('certificates.show', $course->slug) }}" class="su-btn su-btn-solid w-full justify-center mt-4 su-magnetic">
                    ...
                    @else
                        ...
                        <div id="enroll-modal" class="su-modal-overlay hidden items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="enroll-modal-title">
                            <div class="su-modal-panel" role="document">
                                ...
                                <p>You're about to enroll in <strong style="color: var(--c-text);">{{ $course->title }}</strong>.</p>
                                <ul class="su-modal-list">
                                    <li><strong>Modules:</strong> {{ $course->modules->count() }}</li>
                                    <li><strong>Estimated duration:</strong> {{ $course->duration_hours }} hours</li>
                                </ul>
                                <div class="flex gap-3">
                                    <form id="enrollForm" action="{{ route('courses.enroll', $course->slug) }}" method="POST" class="flex-1" aria-labelledby="enroll-modal-title">
                                        @csrf
                    ...
                    {{-- Course Details --}}
                    <div class="mt-2">
                        <div class="su-detail-row"><span>Duration</span><span class="val">{{ $course->duration_hours }} hours</span></div>
                        <div class="su-detail-row"><span>Level</span><span class="val">{{ $course->level }}</span></div>
                        <div class="su-detail-row"><span>Lessons</span><span class="val">{{ $course->lessons->count() }}</span></div>
                        <div class="su-detail-row">
                            <span>Rating</span>
                            <span class="val"><i class="fas fa-star" style="color:#ffcf4d;"></i> {{ number_format($course->rating, 1) }}/5</span>
                        </div>
                    </div>
