...
@section('title', 'My Certificates - SkillUp')
...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 -mt-8 relative z-10">
    ...
    @else
        <div class="mb-6 fade-up flex items-center gap-2 text-sm text-gray-500" style="animation-delay: .05s">
            ...
            <span>{{ $completedEnrollments->count() }} {{ $completedEnrollments->count() === 1 ? 'certificate' : 'certificates' }} earned</span>
        </div>
        ...
        <div class="grid gap-6">
            @foreach($completedEnrollments as $i => $enrollment)
                ...
                @endphp
                <article class="cert-card glass-card rounded-3xl shadow-sm overflow-hidden fade-up" style="animation-delay: {{ 0.08 + ($i * 0.07) }}s">
                    ...
                    <div class="grid gap-6 md:grid-cols-[auto_1fr_auto] items-center p-6 relative z-10">
                        ...
                        <div>
                            <p class="text-sm text-gray-500 flex items-center gap-2">
                                ...
                                {{ $enrollment->completed_at ? $enrollment->completed_at->format('M d, Y') : 'Recently' }}
                            </p>
                            <h2 class="text-2xl font-bold text-gray-900 mt-2">{{ $course->title }}</h2>
                            <p class="text-gray-600 mt-3 flex items-center gap-2 flex-wrap">
                                <span>Certificate ID:</span>
                                <span class="font-mono font-semibold text-gray-900 bg-gray-100 px-2 py-1 rounded-md text-sm tracking-wide">{{ $certificateCode }}</span>
                            </p>
                        ...
                        <div class="flex flex-col gap-3">
                            <a href="{{ route('certificates.show', $course->slug) }}" class="cta-button amber inline-flex items-center justify-center px-6 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white font-semibold shadow-md">
                                <i class="fas fa-print mr-2"></i>
                                View / Print
                            </a>
                            <a href="{{ route('courses.show', $course->slug) }}" class="cta-button inline-flex items-center justify-center px-6 py-3 rounded-xl border border-gray-200 text-gray-700 hover:bg-gray-50">
