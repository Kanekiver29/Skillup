        ...
        <div class="hero-inner">
            ...
            <div class="hero-reveal hero-reveal-2">
                ...
                <div class="hero-status-line">
                    ...
                    <span>·</span>
                    <span><span class="live-count">{{ number_format($courses->total()) }}</span> courses indexed</span>
                </div>
            </div>
            ...
            <p class="hero-subtitle hero-reveal hero-reveal-4">
                Choose from <strong>{{ number_format($courses->total()) }}</strong> expert-crafted courses
            ...
            <div class="hero-stats hero-reveal hero-reveal-5">
                <div class="hero-stat">
                    <span class="val" data-count="{{ $courses->total() }}">{{ number_format($courses->total()) }}</span>
                    <span class="lbl">Courses</span>
                </div>
                <div class="hero-stat">
                    <span class="val" data-count="{{ $categoryCount }}">{{ $categoryCount }}</span>
            ...
            <div class="hero-ctas hero-reveal hero-reveal-6">
                <a href="{{ route('courses.my-learning') }}" class="btn-hero-ghost">
                    <i class="fas fa-book-reader"></i> My Learning
                </a>
                <a href="{{ route('userpage.roadmap') }}" class="btn-hero-solid">
        ...
        <div id="filter-bar" class="filter-bar" data-reveal>
            <form method="GET" action="{{ route('courses.index') }}" id="filter-form">
                <div class="filter-row">
                    ...
                    <div class="filter-group">
                        ...
                        <div style="display: flex; gap: 0.5rem; align-items: center;">
                            ...
                            @if($hasFilters)
                                <a href="{{ route('courses.index') }}" class="btn-clear">
                ...
                <div class="chips-row">
                    ...
                    <a href="{{ route('courses.index', array_merge(request()->except('category', 'page'), ['category' => 'all'])) }}"
                       class="chip {{ $activeCategory === 'all' || !$activeCategory ? 'chip-active' : '' }}">
                        <i class="fas fa-th-large" style="font-size: 0.6rem;"></i> All
                    </a>
                    @foreach($categories->filter() as $cat)
                        <a href="{{ route('courses.index', array_merge(request()->except('category', 'page'), ['category' => $cat])) }}"
    ...
    <div class="results-section">
        ...
        <div class="results-bar" data-reveal>
            <p class="results-count">
                Showing <strong>{{ $courses->count() }}</strong> of
                <strong>{{ number_format($courses->total()) }}</strong> courses
                @if($hasFilters)
        ...
        <div id="courses-grid" class="courses-grid">
            @forelse($courses as $index => $course)
                ...
                <a
                    ...
                    class="course-card"
                    data-reveal
                    data-reveal-delay="{{ min($index % 6 + 1, 5) }}"
                ...
                >
                    ...
                    <div class="card-img-wrap">
                        ...
                        {{-- Badges --}}
                        <span class="card-badge-level">{{ $course->level ?? 'Beginner' }}</span>
                    ...
                    <div class="card-body">
                        <div class="card-meta">
                            ...
                            <span class="card-duration">
                                <i class="fas fa-clock"></i> {{ $course->duration_hours ?? 0 }}h
                            </span>
                        </div>

                        <h3 class="card-title">{{ $course->title }}</h3>
                        <p class="card-desc">{{ $course->short_description }}</p>

                        {{-- Stats --}}
                        <div class="card-stats">
                            <div class="stat-pill">
                                <span class="stat-val">{{ $course->likes_count ?? $course->getLikesCount() }}</span>
                                <span class="stat-key">Likes</span>
                            </div>
                            <div class="stat-pill">
                                <span class="stat-val">{{ number_format($course->students_count ?? 0) }}</span>
                                <span class="stat-key">Learners</span>
                            </div>
                            <div class="stat-pill">
                                <span class="stat-val rating">
                                    {{ number_format($course->rating ?? 0, 1) }}<i class="fas fa-star" style="font-size: 0.6rem; margin-left: 2px;"></i>
                                </span>
                        ...
                        <div class="card-instructor">
                            ...
                            <span class="instructor-name">
                                {{ \Illuminate\Support\Str::limit($course->instructor_name ?? 'SkillUp Trainer', 30) }}
                            </span>
                        </div>
            ...
            @empty
                <div class="empty-state" data-reveal>
                    ...
                    <a href="{{ route('courses.index') }}" class="btn-apply" style="display: inline-flex; text-decoration: none;">
