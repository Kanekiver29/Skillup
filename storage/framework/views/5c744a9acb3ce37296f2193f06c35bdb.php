...
<?php $__env->startSection('title', 'Settings - SkillUp'); ?>
...
<?php $__env->startSection('content'); ?>
	...
	<div class="skillup-settings-page min-h-screen bg-slate-100 py-10 text-slate-800">
		<div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
			...
			<form method="POST" action="<?php echo e(route('user.settings.update')); ?>" class="space-y-6" data-appearance-settings>
				<?php echo csrf_field(); ?>
				...
				<section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
					...
					<div class="grid gap-5 md:grid-cols-2">
						<div class="space-y-2">
							...
							<input id="name" name="name" type="text" value="<?php echo e(old('name', $user->name)); ?>" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 outline-none transition focus:border-cyan-400 focus:bg-white focus:ring-4 focus:ring-cyan-100" required>
						</div>

						<div class="space-y-2">
							<label class="block text-sm font-semibold text-slate-700" for="email">Email address</label>
							<input id="email" name="email" type="email" value="<?php echo e(old('email', $user->email)); ?>" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 outline-none transition focus:border-cyan-400 focus:bg-white focus:ring-4 focus:ring-cyan-100" required>
						</div>

						<div class="space-y-2">
							<label class="block text-sm font-semibold text-slate-700" for="account_phone">Phone number</label>
							<input id="account_phone" name="account[phone]" type="tel" value="<?php echo e(old('account.phone', $accountData['phone'] ?? '')); ?>" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 outline-none transition focus:border-cyan-400 focus:bg-white focus:ring-4 focus:ring-cyan-100" placeholder="+1 (555) 123-4567">
						</div>
						...
						<div class="space-y-2">
							...
							<select id="account_language" name="account[language]" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 outline-none transition focus:border-cyan-400 focus:bg-white focus:ring-4 focus:ring-cyan-100">
								<option value="English" <?php echo e(old('account.language', $accountData['language'] ?? 'English') === 'English' ? 'selected' : ''); ?>>English</option>
								<option value="Filipino" <?php echo e(old('account.language', $accountData['language'] ?? 'English') === 'Filipino' ? 'selected' : ''); ?>>Filipino</option>
								<option value="Spanish" <?php echo e(old('account.language', $accountData['language'] ?? 'English') === 'Spanish' ? 'selected' : ''); ?>>Spanish</option>
							</select>
						</div>
					</div>
				</section>
					...
					<div class="grid gap-5 md:grid-cols-2">
						<div class="space-y-2">
							...
							<select id="learning_style" name="learning_preferences[learning_style]" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 outline-none transition focus:border-cyan-400 focus:bg-white focus:ring-4 focus:ring-cyan-100">
								<option value="visual" <?php echo e(old('learning_preferences.learning_style', $learningData['learning_style'] ?? 'balanced') === 'visual' ? 'selected' : ''); ?>>Visual</option>
								<option value="auditory" <?php echo e(old('learning_preferences.learning_style', $learningData['learning_style'] ?? 'balanced') === 'auditory' ? 'selected' : ''); ?>>Auditory</option>
								<option value="kinesthetic" <?php echo e(old('learning_preferences.learning_style', $learningData['learning_style'] ?? 'balanced') === 'kinesthetic' ? 'selected' : ''); ?>>Hands-on</option>
								<option value="balanced" <?php echo e(old('learning_preferences.learning_style', $learningData['learning_style'] ?? 'balanced') === 'balanced' ? 'selected' : ''); ?>>Balanced</option>
							</select>
						</div>
						...
						<div class="space-y-2">
							...
							<select id="learning_pace" name="learning_preferences[pace]" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 outline-none transition focus:border-cyan-400 focus:bg-white focus:ring-4 focus:ring-cyan-100">
								<option value="steady" <?php echo e(old('learning_preferences.pace', $learningData['pace'] ?? 'steady') === 'steady' ? 'selected' : ''); ?>>Steady</option>
								<option value="fast" <?php echo e(old('learning_preferences.pace', $learningData['pace'] ?? 'steady') === 'fast' ? 'selected' : ''); ?>>Fast</option>
								<option value="deep" <?php echo e(old('learning_preferences.pace', $learningData['pace'] ?? 'steady') === 'deep' ? 'selected' : ''); ?>>Deep dive</option>
							</select>
						</div>
						...
						<div class="md:col-span-2 space-y-2">
							...
							<select id="learning_tracks" name="learning_preferences[tracks][]" multiple class="min-h-[120px] w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 outline-none transition focus:border-cyan-400 focus:bg-white focus:ring-4 focus:ring-cyan-100">
								<?php $selectedTracks = old('learning_preferences.tracks', $learningData['tracks'] ?? []); ?>
								<option value="Web Development" <?php echo e(is_array($selectedTracks) && in_array('Web Development', $selectedTracks, true) ? 'selected' : ''); ?>>Web Development</option>
								<option value="Data Science" <?php echo e(is_array($selectedTracks) && in_array('Data Science', $selectedTracks, true) ? 'selected' : ''); ?>>Data Science</option>
								<option value="Design" <?php echo e(is_array($selectedTracks) && in_array('Design', $selectedTracks, true) ? 'selected' : ''); ?>>Design</option>
								<option value="Business" <?php echo e(is_array($selectedTracks) && in_array('Business', $selectedTracks, true) ? 'selected' : ''); ?>>Business</option>
							</select>
						</div>
					</div>
				</section>
					...
					<div class="grid gap-5 md:grid-cols-3">
						<div class="space-y-2">
							...
							<select id="appearance_theme" name="appearance[theme]" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 outline-none transition focus:border-cyan-400 focus:bg-white focus:ring-4 focus:ring-cyan-100">
								<option value="light" <?php echo e(old('appearance.theme', $appearanceData['theme'] ?? 'light') === 'light' ? 'selected' : ''); ?>>Light</option>
								<option value="dark" <?php echo e(old('appearance.theme', $appearanceData['theme'] ?? 'light') === 'dark' ? 'selected' : ''); ?>>Dark</option>
								<option value="system" <?php echo e(old('appearance.theme', $appearanceData['theme'] ?? 'light') === 'system' ? 'selected' : ''); ?>>System</option>
							</select>
						</div>
						...
						<div class="space-y-2">
							...
							<select id="appearance_font_size" name="appearance[font_size]" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 outline-none transition focus:border-cyan-400 focus:bg-white focus:ring-4 focus:ring-cyan-100">
								<option value="small" <?php echo e(old('appearance.font_size', $appearanceData['font_size'] ?? 'medium') === 'small' ? 'selected' : ''); ?>>Small</option>
								<option value="medium" <?php echo e(old('appearance.font_size', $appearanceData['font_size'] ?? 'medium') === 'medium' ? 'selected' : ''); ?>>Medium</option>
								<option value="large" <?php echo e(old('appearance.font_size', $appearanceData['font_size'] ?? 'medium') === 'large' ? 'selected' : ''); ?>>Large</option>
							</select>
						</div>
						...
						<label class="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
							...
							<input type="checkbox" name="appearance[reduced_motion]" value="1" <?php echo e(old('appearance.reduced_motion', $appearanceData['reduced_motion'] ?? false) ? 'checked' : ''); ?> class="h-5 w-5 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500">
						</label>
					</div>
				</section>
				...
				<section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
					...
					<div class="grid gap-4 md:grid-cols-3">
						<label class="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
							...
							<input type="checkbox" name="notifications[email]" value="1" <?php echo e(old('notifications.email', $notificationsData['email'] ?? true) ? 'checked' : ''); ?> class="h-5 w-5 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500">
						</label>

						<label class="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
							<span class="text-sm font-medium text-slate-800">Push alerts</span>
							<input type="checkbox" name="notifications[push]" value="1" <?php echo e(old('notifications.push', $notificationsData['push'] ?? true) ? 'checked' : ''); ?> class="h-5 w-5 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500">
						</label>

						<label class="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
							<span class="text-sm font-medium text-slate-800">Weekly digest</span>
							<input type="checkbox" name="notifications[weekly_digest]" value="1" <?php echo e(old('notifications.weekly_digest', $notificationsData['weekly_digest'] ?? false) ? 'checked' : ''); ?> class="h-5 w-5 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500">
						</label>
					</div>
				</section>
					...
					<div class="grid gap-5 md:grid-cols-2">
						<div class="space-y-2">
							...
							<select id="privacy_visibility" name="privacy[profile_visibility]" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-slate-900 outline-none transition focus:border-cyan-400 focus:bg-white focus:ring-4 focus:ring-cyan-100">
								<option value="public" <?php echo e(old('privacy.profile_visibility', $privacyData['profile_visibility'] ?? 'public') === 'public' ? 'selected' : ''); ?>>Public</option>
								<option value="students_only" <?php echo e(old('privacy.profile_visibility', $privacyData['profile_visibility'] ?? 'public') === 'students_only' ? 'selected' : ''); ?>>Students only</option>
								<option value="private" <?php echo e(old('privacy.profile_visibility', $privacyData['profile_visibility'] ?? 'public') === 'private' ? 'selected' : ''); ?>>Private</option>
							</select>
						</div>
						...
						<label class="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
							...
							<input type="checkbox" name="privacy[show_contact]" value="1" <?php echo e(old('privacy.show_contact', $privacyData['show_contact'] ?? false) ? 'checked' : ''); ?> class="h-5 w-5 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500">
						</label>
					</div>
				</section>
				...
				<section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
					...
					<div class="grid gap-4 md:grid-cols-2">
						<label class="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
							...
							<input type="checkbox" name="security[two_factor]" value="1" <?php echo e(old('security.two_factor', $securityData['two_factor'] ?? false) ? 'checked' : ''); ?> class="h-5 w-5 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500">
						</label>
						...
						<label class="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 p-4">
							...
							<input type="checkbox" name="security[login_alerts]" value="1" <?php echo e(old('security.login_alerts', $securityData['login_alerts'] ?? true) ? 'checked' : ''); ?> class="h-5 w-5 rounded border-slate-300 text-cyan-600 focus:ring-cyan-500">
						</label>
					</div>
				</section>

				<div class="flex justify-end gap-3 pt-2">
					<a href="<?php echo e(route('userpage.dashboard')); ?>" class="inline-flex items-center rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-400 hover:bg-slate-50">Cancel</a>
...
<?php echo $__env->make('layout.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\skillupv2\resources\views\Userpage\setting\setting.blade.php ENDPATH**/ ?>