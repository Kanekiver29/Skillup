            ...
            <div class="xl:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                    ...
                    <div class="flex flex-wrap gap-3">
                        ...
                        <form action="<?php echo e(route('admin.backup.store')); ?>" method="POST" class="inline-flex">
                            <?php echo csrf_field(); ?>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            <?php $__empty_1 = true; $__currentLoopData = $backupFiles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $file): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="hover:bg-slate-50 transition">
                                    <td class="px-5 py-4 text-gray-900 font-medium">
                                        <i class="fas <?php echo e($file['type'] === 'Full Backup' ? 'fa-box' : 'fa-database'); ?> text-gray-400 mr-2"></i>
                                        <?php echo e($file['name']); ?>

                                    </td>
                                    <td class="px-5 py-4 text-gray-700">
                                        <span class="inline-flex items-center gap-1 rounded-full <?php echo e($file['type'] === 'Full Backup' ? 'bg-emerald-100 text-emerald-800 px-3 py-1' : 'bg-cyan-100 text-cyan-800 px-3 py-1'); ?> text-xs font-semibold">
                                            <?php echo e($file['type']); ?>

                                        </span>
                                    </td>
                                    <td class="px-5 py-4 text-gray-700">
                                        <?php if($file['size'] > 1024*1024*1024): ?>
                                            <?php echo e(number_format($file['size'] / (1024*1024*1024), 2)); ?> GB
                                        <?php elseif($file['size'] > 1024*1024): ?>
                                            <?php echo e(number_format($file['size'] / (1024*1024), 2)); ?> MB
                                        <?php else: ?>
                                            <?php echo e(number_format($file['size'] / 1024, 2)); ?> KB
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-5 py-4 text-gray-700"><?php echo e(\Carbon\Carbon::createFromTimestamp($file['modified'])->format('M d, Y H:i')); ?></td>
                                    <td class="px-5 py-4 text-right space-x-2">
                                        <a href="<?php echo e(route('admin.backup.download', urlencode($file['name']))); ?>" class="inline-flex items-center gap-2 rounded-lg border border-cyan-100 bg-cyan-50 px-4 py-2 text-xs font-semibold text-cyan-700 hover:bg-cyan-100">
                                            <i class="fas fa-download"></i>
                                            Download
                                        </a>
                                                        <?php if($file['type'] === 'Database'): ?>
                                            <form action="<?php echo e(route('admin.backup.restore')); ?>" method="POST" class="inline-block">
                                                <?php echo csrf_field(); ?>
                                                <input type="hidden" name="backup_file" value="<?php echo e($file['name']); ?>">
                                                <button type="submit" onclick="return confirm('Restore database from <?php echo e($file['name']); ?>? This will overwrite current data.')" class="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-xs font-semibold text-white hover:bg-slate-800">
                                                    <i class="fas fa-undo"></i>
                                                    Restore
                                                </button>
                                            </form>
                                        <?php endif; ?>
                                        <?php if($file['type'] === 'Full Backup'): ?>
                                            <form action="<?php echo e(route('admin.backup.restore')); ?>" method="POST" class="inline-block">
                                                <?php echo csrf_field(); ?>
                                                <input type="hidden" name="backup_file" value="<?php echo e($file['name']); ?>">
                                                ...
                                                <button type="submit" onclick="return confirm('Perform full restore from <?php echo e($file['name']); ?>? This will overwrite application files. Proceed?')" class="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-xs font-semibold text-white hover:bg-slate-800">
                                                    <i class="fas fa-undo"></i>
                                                    Restore
                                                </button>
                                            </form>
                ...
                <?php if(isset($diagnostics)): ?>
                    ...
                    <?php if($diagnostics['log_tail']): ?>
                        <div class="rounded-lg border border-rose-100 bg-rose-50 p-3">
                            ...
                            <pre class="mt-2 text-xs text-rose-900 whitespace-pre-wrap"><?php echo e($diagnostics['log_tail']); ?></pre>
                        </div>
                    <?php endif; ?>
                <?php endif; ?>
...
<?php echo $__env->make('layout.Admin.system', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\skilluplastest\resources\views/Admin/backup.blade.php ENDPATH**/ ?>