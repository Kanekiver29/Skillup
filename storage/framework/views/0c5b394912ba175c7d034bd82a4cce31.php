<?php $__env->startSection('title', 'Create New Syllabus'); ?>

<?php $__env->startSection('content'); ?>
<style>
/* ═══════════════════════════════════════════════════════
   SYLLABI CREATE — Ultra-polished console UI
   ═══════════════════════════════════════════════════════ */
.syl-wrap {
    --bg:      var(--body-bg);
    --surface: var(--surface);
...
.section-card:hover { border-color:rgba(34,211,238,0.28); }
.section-card:hover::before { opacity:1; }
.section-card:hover { box-shadow:0 0 20px rgba(34,211,238,0.05); }
...
/* section count badge */
.sec-count-badge {
    font-size:0.68rem; font-family:monospace; color:var(--dim);
...
.media-zone:hover, .media-zone.dragover {
...
.media-counter {
    ...
    background:rgba(34,211,238,0.09); color:var(--cyan);
    border:1px solid rgba(34,211,238,0.18);
}
.media-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(68px,1fr)); gap:6px; margin-top:10px; }
.media-thumb {
    position:relative; border-radius:8px; overflow:hidden;
    border:1px solid var(--border); background:rgba(8,13,24,0.5);
    animation:th-in .22s cubic-bezier(.22,.61,.36,1) both;
}
@keyframes th-in { from{opacity:0;transform:scale(.85);} to{opacity:1;transform:scale(1);} }
.media-thumb img { width:100%; height:68px; object-fit:cover; display:block; }
.media-thumb-video {
...
.media-thumb-rm:hover { background:rgba(239,68,68,0.85); }

@media (prefers-reduced-motion:reduce) {
    .syl-orb,.syl-scanline,.syl-heading,.syl-field,.section-card,.media-thumb,.syl-btn-primary::after {
...
<div class="syl-wrap" style="position:relative; min-height:100vh; padding:1.5rem 1.5rem 3rem;">
    ...
    <div style="max-width:760px; margin:0 auto; position:relative; z-index:2;">
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1.5rem;">
            <div>
                <h1 class="syl-heading">Create New Syllabus</h1>
                <p class="syl-sub">Fill in the details and sections below.</p>
            </div>
            <a href="<?php echo e(route('staff.syllabi.index')); ?>" class="syl-back">
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($errors->any()): ?>
            <div class="syl-err">
                <ul class="list-disc list-inside text-sm" style="line-height:1.8;">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $e): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?><li><?php echo e($e); ?></li><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                </ul>
            </div>
        ...
        <div class="syl-panel">
            ...
            <div class="syl-toolbar">
                <div style="display:flex; align-items:center;">
                    ...
                    <span class="syl-toolbar-title">syllabi.create</span>
                </div>
            ...
            <div class="syl-panel-content">
                <form action="<?php echo e(route('staff.syllabi.store')); ?>" method="POST"
                    ...
                    <div class="syl-field" style="--fi:2; margin-top:1rem;">
                        ...
                        <textarea name="description" id="description" rows="3"
                                  placeholder="Brief overview of this syllabus…"
                                  class="syl-textarea"><?php echo e(old('description')); ?></textarea>
                    </div>
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-top:1rem;" class="responsive-grid">
                        <div class="syl-field" style="--fi:3;">
                            ...
                            <input type="date" name="effective_start" id="effective_start"
                                   value="<?php echo e(old('effective_start')); ?>" class="syl-input"/>
                        </div>
                        <div class="syl-field" style="--fi:4;">
                            ...
                            <input type="date" name="effective_end" id="effective_end"
                                   value="<?php echo e(old('effective_end')); ?>" class="syl-input"/>
                        </div>
                    </div>
                    ...
                    <div class="syl-field" style="--fi:5; margin-top:1.1rem;">
                        <label class="syl-toggle">
                            ...
                            <span class="syl-toggle-lbl">Publish immediately</span>
                            <span style="font-size:0.72rem; color:var(--dim);">(makes this visible to students)</span>
                        </label>
                    </div>
                    ...
                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:1rem;">
                        ...
                        <span class="sec-count-badge" id="sec-count">1 / 10</span>
                    </div>

                    <div id="sections-wrapper" style="display:flex; flex-direction:column; gap:0.65rem;">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(old('sections')): ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = old('sections'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $section): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                                <div class="section-card" data-index="<?php echo e($i); ?>">
                                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:0.65rem;">
                                        <div style="display:flex; align-items:center; gap:6px;">
                                            <span class="sec-num"><?php echo e($i + 1); ?></span>
                                    ...
                                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.75rem;" class="responsive-grid">
                                        <div>
                                            ...
                                            <input type="text" name="sections[<?php echo e($i); ?>][section_title]"
                                                   value="<?php echo e($section['section_title'] ?? ''); ?>"
                                                   class="syl-input" placeholder="e.g. Introduction" required/>
                                        </div>
                                        <div>
                                            <label class="syl-label">Content</label>
                                            <textarea name="sections[<?php echo e($i); ?>][content]" rows="2"
                                                      class="syl-textarea" placeholder="Describe this section…"><?php echo e($section['content'] ?? ''); ?></textarea>
                                        </div>
                                    </div>
                                </div>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                        <?php else: ?>
                            <div class="section-card" data-index="0">
                                ...
                                <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.75rem;" class="responsive-grid">
                                    ...
                                    <div>
                                        <label class="syl-label">Content</label>
                                        <textarea name="sections[0][content]" rows="2"
                                                  class="syl-textarea" placeholder="Describe this section…"></textarea>
                                    </div>
                                </div>
                            </div>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:1rem;" class="responsive-grid">
                        ...
                        <div class="syl-field" style="--fi:6;">
                            ...
                            <div class="media-zone" id="photo-drop">
                                ...
                                <p style="font-size:0.75rem; color:var(--dim);">Click or drop images</p>
                                <p style="font-size:0.68rem; color:#374151; margin-top:2px;">Max 25 · 5 MB each</p>
                            </div>
                        ...
                        <div class="syl-field" style="--fi:7;">
                            ...
                            <div class="media-zone" id="video-drop">
                                ...
                                <p style="font-size:0.75rem; color:var(--dim);">Click or drop videos</p>
                                <p style="font-size:0.68rem; color:#374151; margin-top:2px;">Max 3 · 20 MB · MP4/WebM/OGG</p>
                            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function refreshCount() {
        ...
        var cards = wrapper.querySelectorAll('.section-card');
    ...
    function makeCard(i) {
        ...
        d.innerHTML =
            '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.65rem;">' +
                ...
                '</div>' +
                '<button type="button" class="remove-section syl-btn-rm">' +
                    '<svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg> Remove' +
                '</button>' +
            '</div>' +
            '<div style="display:grid;grid-template-columns:1fr 1fr;gap:0.75rem;" class="responsive-grid">' +
                '<div><label class="syl-label">Section Title</label>' +
                '<input type="text" name="sections[' + i + '][section_title]" class="syl-input" placeholder="e.g. Introduction" required/></div>' +
                '<div><label class="syl-label">Content</label>' +
                '<textarea name="sections[' + i + '][content]" rows="2" class="syl-textarea" placeholder="Describe this section…"></textarea></div>' +
            '</div>';
    ...
    addBtn.addEventListener('click', function () {
        if (wrapper.children.length >= MAX_SEC) return;
        makeCard(secIdx++);
    });
    ...
    function photoCounterHTML(n) {
        return '<svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>' + n + ' / 25';
    }
    ...
    function renderPhotos() {
        ...
        photoFiles.forEach(function (f, i) {
            ...
            var rm = document.createElement('button'); rm.type='button'; rm.className='media-thumb-rm'; rm.dataset.index=i; rm.textContent='×';
            wrap.appendChild(img); wrap.appendChild(rm); photoPreviews.appendChild(wrap);
        });
        photoCounter.innerHTML = photoCounterHTML(photoFiles.length);
    }
    photoInput.addEventListener('change', function () {
        Array.from(photoInput.files).forEach(function (f) { if (photoFiles.length < MAX_PHOTOS) photoFiles.push(f); });
        syncPhotos(); renderPhotos();
    });
    photoPreviews.addEventListener('click', function (e) {
        var btn = e.target.closest('.media-thumb-rm'); if (!btn) return;
    ...
    ['dragover','dragleave','drop'].forEach(function (ev) {
        photoDrop.addEventListener(ev, function (e) {
            ...
            if (ev === 'drop') { Array.from(e.dataTransfer.files).filter(function(f){return f.type.startsWith('image/');}).forEach(function(f){if(photoFiles.length<MAX_PHOTOS)photoFiles.push(f);}); syncPhotos(); renderPhotos(); }
        });
    });
    ...
    function videoCounterHTML(n) {
        return '<svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>' + n + ' / 3';
    }
    ...
    function renderVideos() {
        ...
        videoCounter.innerHTML = videoCounterHTML(videoFiles.length);
    }
    videoInput.addEventListener('change', function () {
        Array.from(videoInput.files).forEach(function (f) { if (videoFiles.length < MAX_VIDEOS) videoFiles.push(f); });
    ...
    ['dragover','dragleave','drop'].forEach(function (ev) {
        videoDrop.addEventListener(ev, function (e) {
            ...
            if (ev === 'drop') { Array.from(e.dataTransfer.files).filter(function(f){return f.type.startsWith('video/');}).forEach(function(f){if(videoFiles.length<MAX_VIDEOS)videoFiles.push(f);}); syncVideos(); renderVideos(); }
        });
    });
});
</script>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('staff.layouts.masters', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\skillupv2\resources\views\staff\sysllabus\create.blade.php ENDPATH**/ ?>