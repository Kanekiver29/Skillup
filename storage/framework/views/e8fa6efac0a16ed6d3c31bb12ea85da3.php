        ...
        <div class="quiz-page-header mb-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="quiz-actions flex-shrink-0">
                    <a href="<?php echo e(route('staff.quizzes.create')); ?>"
            ...
            <div class="quiz-stats-card card rounded-3xl border border-slate-200 bg-slate-50 p-6 shadow-sm">
                ...
                <div class="mt-5 quiz-stat-grid">
                    <div class="quiz-stat-block">
                        ...
                        <span class="quiz-stat-value" data-count-to="<?php echo e($quizzes->count()); ?>">0</span>
                    </div>
                    <div class="quiz-stat-block">
                        <span class="quiz-stat-label">Modules covered</span>
                        <span class="quiz-stat-value" data-count-to="<?php echo e($quizzes->pluck('module.title')->unique()->filter()->count()); ?>">0</span>
                    </div>
                    <div class="quiz-stat-block">
                        <span class="quiz-stat-label">Avg passing score</span>
                        <span class="quiz-stat-value" data-count-to="<?php echo e(round($quizzes->avg('passing_score') ?? 0)); ?>">0</span>
                    </div>
                </div>
            </div>
        ...
        <div class="quiz-card overflow-hidden rounded-[28px]">
            <div class="grid gap-4 border-b border-slate-200/70 bg-slate-50/90 p-5 backdrop-blur-sm sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                ...
                <div class="flex flex-wrap items-center justify-end gap-2">
                    <span id="quiz-total-pill" class="quiz-pill"><i class="fas fa-check-circle text-sky-600" aria-hidden="true"></i> <?php echo e($quizzes->count()); ?> quizzes</span>
                    <span id="quiz-modules-pill" class="quiz-pill secondary"><i class="fas fa-layer-group text-slate-500" aria-hidden="true"></i> <?php echo e($quizzes->pluck('module.title')->unique()->filter()->count()); ?> modules</span>
                    <?php if($archivedCount > 0): ?>
                        <a href="<?php echo e(route('staff.quizzes.archived')); ?>" class="quiz-pill secondary" title="View archived quizzes">
                            <i class="fas fa-archive text-slate-500" aria-hidden="true"></i>
                            <?php echo e($archivedCount); ?> archived
                        </a>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <table class="quiz-table min-w-full divide-y divide-slate-200">
                ...
                <tbody class="divide-y divide-slate-200" id="quiz-table-body">
                    <?php $__empty_1 = true; $__currentLoopData = $quizzes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $quiz): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        ...
                        ?>
                        <tr class="quiz-row"
                            style="animation-delay: <?php echo e($loop->index * 0.05); ?>s;"
                            data-search="<?php echo e(strtolower($quiz->title . ' ' . $moduleTitle)); ?>"
                            data-module="<?php echo e($moduleTitle); ?>">
                            <td class="px-6 py-4 text-sm font-semibold text-slate-900">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="quiz-title-text"><?php echo e($quiz->title); ?></div>
                                        <div class="quiz-desc-text text-xs text-slate-500"><?php echo e(Str::limit($quiz->description ?? 'No description', 55)); ?></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-600"><?php echo e($moduleTitle); ?></td>
                            <td class="px-6 py-4 text-sm text-slate-600">
                                <span class="quiz-badge inline-flex items-center rounded-full <?php echo e($badgeClasses); ?> px-3 py-1 text-xs font-semibold">
                                    <?php echo e($quiz->passing_score); ?>%
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-500"><?php echo e($quiz->created_at->format('M d, Y')); ?></td>
                            <td class="px-6 py-4 text-right text-sm">
                                <a href="<?php echo e(route('staff.quizzes.questionnaire', $quiz)); ?>" class="quiz-action secondary font-semibold mr-3">
                                    <i class="fas fa-list-check mr-1" aria-hidden="true"></i>Questions
                                </a>
                                <a href="<?php echo e(route('staff.quizzes.edit', $quiz)); ?>" class="quiz-action secondary font-semibold mr-3">
                                    <i class="fas fa-pen-to-square mr-1" aria-hidden="true"></i>Edit
                                </a>
                                <form id="quiz-archive-form-<?php echo e($quiz->id); ?>" action="<?php echo e(route('staff.quizzes.archive', $quiz)); ?>" method="POST" class="inline">
                                    <?php echo csrf_field(); ?>
                                ...
                                <button type="button"
                                        ...
                                        data-quiz-title="<?php echo e($quiz->title); ?>">
                                    <i class="fas fa-trash mr-1" aria-hidden="true"></i>Delete
                                </button>
                                <form id="quiz-delete-form-<?php echo e($quiz->id); ?>" action="<?php echo e(route('staff.quizzes.destroy', $quiz)); ?>" method="POST" class="hidden">
                                    <?php echo csrf_field(); ?>
                        ...
                        <tr>
                            <td colspan="5" class="quiz-empty px-6 py-20 text-center text-sm text-slate-500">
                                <div class="quiz-empty-state mx-auto max-w-md overflow-hidden rounded-[28px] border border-dashed border-slate-200 bg-gradient-to-br from-slate-50 via-slate-100 to-white p-10 shadow-sm">
                                    ...
                                    <a href="<?php echo e(route('staff.quizzes.create')); ?>" class="quiz-button is-primary inline-flex items-center gap-2 rounded-2xl bg-sky-600 px-5 py-3 text-sm font-semibold text-white shadow hover:bg-sky-700">
                                        <i class="fas fa-plus" aria-hidden="true"></i>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('staff.layouts.masters', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\skilluplastest\resources\views/staff/quizzes/list.blade.php ENDPATH**/ ?>