...
<?php $__env->startSection('title', 'Database - SkillUp Admin'); ?>
	...
	<div class="bg-white border-b border-gray-200 mb-8">
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
			...
			<div class="text-sm text-gray-600 bg-cyan-50 border border-cyan-100 rounded-lg px-4 py-3">
				<p class="font-medium text-cyan-700">Last sync: <?php echo e($lastUpdated->format('M d, Y h:i A')); ?></p>
				<p class="text-cyan-600">Connection: <?php echo e(strtoupper($connection)); ?> (<?php echo e(strtoupper($driver)); ?>)</p>
			</div>
		</div>
	</div>

	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 space-y-6">
		<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!$connectionStatus): ?>
			<div class="bg-red-50 border border-red-200 rounded-lg p-5">
				<div class="flex items-start gap-3">
					<i class="fas fa-exclamation-triangle text-red-500 mt-0.5"></i>
					<div>
						<h2 class="text-base font-semibold text-red-800">Database connection failed</h2>
						<p class="text-sm text-red-700 mt-1"><?php echo e($connectionError); ?></p>
					</div>
				</div>
			</div>
		...
		<section class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
			<article class="bg-white border border-gray-100 rounded-xl shadow-sm p-5">
				...
				<p class="text-xl font-bold text-gray-900 break-all"><?php echo e($databaseName); ?></p>
			...
			<article class="bg-white border border-gray-100 rounded-xl shadow-sm p-5">
				...
				<p class="text-3xl font-bold text-gray-900"><?php echo e(number_format($totalTables)); ?></p>
			...
			<article class="bg-white border border-gray-100 rounded-xl shadow-sm p-5">
				...
				<p class="text-3xl font-bold text-gray-900"><?php echo e(number_format($totalRows)); ?></p>
			...
			<article class="bg-white border border-gray-100 rounded-xl shadow-sm p-5">
				...
				<p class="text-3xl font-bold text-gray-900"><?php echo e(number_format($totalSizeMb, 2)); ?> MB</p>
		...
		<section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
			<div class="lg:col-span-2 bg-white border border-gray-100 rounded-xl shadow-sm overflow-hidden">
				<div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
					...
					<span class="text-xs font-medium px-2.5 py-1 rounded-full <?php echo e($connectionStatus ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'); ?>">
						<?php echo e($connectionStatus ? 'Connected' : 'Disconnected'); ?>
				...
				<div class="overflow-x-auto">
					<table class="min-w-full divide-y divide-gray-200 text-sm">
						...
						<tbody class="divide-y divide-gray-100 bg-white">
							<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $tableStats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $table): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
								<tr class="hover:bg-gray-50">
									<td class="px-6 py-3 font-medium text-gray-900"><?php echo e($table['name']); ?></td>
									<td class="px-6 py-3 text-gray-600"><?php echo e($table['engine']); ?></td>
									<td class="px-6 py-3 text-right text-gray-700"><?php echo e(number_format($table['rows'])); ?></td>
									<td class="px-6 py-3 text-right text-gray-700"><?php echo e(number_format($table['size_mb'], 2)); ?> MB</td>
									<td class="px-6 py-3 text-gray-600">
										<?php echo e($table['updated_at'] ? \Illuminate\Support\Carbon::parse($table['updated_at'])->format('M d, Y h:i A') : 'N/A'); ?>
			...
			<div class="space-y-6">
				<div class="bg-white border border-gray-100 rounded-xl shadow-sm p-6">
					...
					<div class="flex items-center justify-between mb-4">
						...
						<span class="px-2.5 py-1 rounded-full text-xs font-medium <?php echo e($usageClass); ?>"><?php echo e($usageState); ?></span>
					</div>
					<div class="w-full bg-gray-200 rounded-full h-2.5 mb-2">
						<div class="h-2.5 rounded-full <?php echo e($usageState === 'Healthy' ? 'bg-green-500' : ($usageState === 'Watch' ? 'bg-yellow-500' : 'bg-red-500')); ?>"
							 style="width: <?php echo e(min(100, max(6, ($totalSizeMb / 2048) * 100))); ?>%"></div>
					</div>
		...
		<section class="bg-linear-to-r from-slate-800 to-slate-700 rounded-xl p-6 text-white shadow-sm">
			<div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
				...
				<div class="flex items-center gap-3">
					...
					<a href="<?php echo e(route('admin.settings')); ?>" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-white text-sm font-medium transition">
...
<?php echo $__env->make('layout.Admin.system', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\skillupv2\resources\views\Admin\database.blade.php ENDPATH**/ ?>