...
<div class="container py-8">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Editing: <span class="font-semibold text-slate-700"><?php echo e($module->title); ?></span></p>
        </div>

        <div class="flex items-center gap-3">
            <a href="<?php echo e(route('staff.modules.index', ['course_id' => $module->course_id])); ?>" class="inline-flex items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-slate-400 hover:bg-slate-50">
                Back to Modules
            </a>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <form action="<?php echo e(route('staff.modules.update', $module)); ?>" method="POST">
            ...
            <div class="mt-6 flex flex-wrap items-center gap-3">
                ...
                <a href="<?php echo e(route('staff.modules.index', ['course_id' => $module->course_id])); ?>" class="inline-flex items-center justify-center rounded-lg border border-slate-300 bg-white px-5 py-2.5 text-sm font-medium text-slate-700 transition hover:border-slate-400 hover:bg-slate-50">
                    Cancel
                </a>
            </div>
        </form>
    ...
    <div class="mt-8 max-w-2xl rounded-2xl border border-amber-200 bg-amber-50 p-6">
        ...
        <form action="<?php echo e(route('staff.modules.destroy', $module)); ?>" method="POST" class="mt-4" onsubmit="return confirm('Are you sure you want to archive this module?');">
            <?php echo csrf_field(); ?>
    ...
    <div class="mt-4 max-w-2xl rounded-2xl border border-red-200 bg-red-50 p-6">
        ...
        <form action="<?php echo e(route('staff.modules.destroy', $module)); ?>" method="POST" class="mt-4" onsubmit="return confirm('Are you sure? This will PERMANENTLY delete this module and all its data. This cannot be undone.');">
            <?php echo csrf_field(); ?>
            <?php echo method_field('DELETE'); ?>
...
<?php echo $__env->make('staff.layouts.masters', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\skillupv2\resources\views\staff\modules\edit.blade.php ENDPATH**/ ?>