            ...
            <div x-show="tab === 'maintenance'" x-transition>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
                        ...
                        <form action="<?php echo e(route('admin.settings.maintenance')); ?>" method="POST" class="mt-4 ajax-action-form" data-action-type="maintenance">
                            <?php echo csrf_field(); ?>
                            <label for="maintenance_reason" class="block text-sm font-medium text-gray-700 mb-2">Specific maintenance details</label>
                            <textarea id="maintenance_reason" name="maintenance_reason" rows="4" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500" placeholder="Example: Upgrading the student database and syncing course records from 10:00 PM to 12:00 AM."><?php echo e(old('maintenance_reason', env('APP_MAINTENANCE_REASON', 'Scheduled system maintenance in progress.'))); ?></textarea>
                            <button type="submit" onclick="return confirm('<?php echo e(app()->isDownForMaintenance() ? 'Disable public maintenance mode?' : 'Enable maintenance for public site?'); ?>')" class="mt-4 w-full px-4 py-3 rounded-lg <?php echo e(app()->isDownForMaintenance() ? 'bg-green-600 text-white' : 'bg-red-600 text-white'); ?>"><?php echo e(app()->isDownForMaintenance() ? 'Disable Public Maintenance' : 'Enable Maintenance for Public Site'); ?></button>
                        </form>
                    </div>
                    <div class="space-y-4">
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-5">
                            ...
                            <p class="mt-2 text-sm text-blue-700"><?php echo e(env('APP_MAINTENANCE_REASON', 'Scheduled system maintenance in progress.')); ?></p>
...
<?php echo $__env->make('layout.Admin.system', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\skillupv2\resources\views\Admin\settings.blade.php ENDPATH**/ ?>